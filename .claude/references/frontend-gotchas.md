...
- **`capture` timed out after 60 s for two separate reasons, both from the 2026-09-23 webcraft trial and both reproduced in headless Chrome.** (1) snapdom runs inside the iframe's own window (`snapdomFor`); when that document is replaced mid-shot — an agent edits a file and captures at once, the `content_update` lands a moment later and the viewer reloads the page — the discarded window's promises never settle (measured: still pending after 15 s), so the capture never answered. `shootIframePage` now races each shot against the iframe's `load` / `aria-busy` and re-shoots the new page (bounded). (2) A background tab gets no animation frames, and after five minutes hidden Chrome aligns chained timers (nesting ≥ 5) to one wake-up per minute: a replica of `primeScrollReveals` on the Gazette measured `[230, 1230, …, 5230, 22233, 82230, 142230] ms`. Nothing in the capture path may poll or chain sleeps (the snapdom injection now waits on the script's `load`/`error`), and reveal priming is skipped when the page is hidden, with `HIDDEN_TAB_NOTE` in the result — IntersectionObserver cannot fire there however long you wait. snapdom itself is fine hidden (0.5 s for the Gazette front page). Reproduce with `document.visibilityState === "hidden"` for over five minutes before judging a capture change.

- **snapdom loses `<html lang>`, so `hyphens: auto` stops hyphenating in the capture** (2026-09-23, the Gazette's "overlapping lines" in justified CSS columns): the clone root is `<body>` or the element, not `<html>`, lines come out longer than the page's own and spill past the heights snapdom froze. `snapdomToPng` sets the inherited `lang` on the captured root for the shot. Against a native full-page screenshot at 1280 px: 78,794 differing pixels without it, 21,697 with it; the remainder is table row metrics and the page's own animation state, not text overlap. Captures now also wait for `document.fonts.ready`: layout measured on a fallback font and painted with the embedded web font is the other way to get overlapping lines.

- **An iframe viewer whose page is loading marks the iframe `aria-busy="true"` until the page document's `load`, and reports `onNavigateComplete` only then** (webcraft: set when it sends the iframe somewhere, and by the page itself on `pagehide` — a reload, a link, a script setting `location`; cleared in the parent's `load` handler after instrumenting). `capture` waits on both; before, it slept a fixed 1.1 s and shot whatever was there, including a page still loading or the one being left. Do not shortcut on `document.readyState === "complete"`: during a navigation the iframe still holds the old, complete document (the 2026-09-23 review caught arrival being reported before the target's images had loaded).
...
- **Show a same-origin site page by navigating the iframe to its real URL, not by writing a document into it** (2026-09-23, webcraft). A `srcdoc` document's URL is `about:srcdoc`, and `history.replaceState` only lets an `about:` URL change its fragment (`SecurityError` for `?…`, Chrome 153), so a page never sees its query; a blob-URL document refuses the query too and is not service-worker controlled. A `document.open()`-written document fixed the URL but not the rest (review of the same day): its requests bypassed the hosted player's service worker (images, stylesheets and `fetch` failed in `/s/<id>`), a `location.reload()` loaded the raw file without the viewer's scripts (selection dead, page tabs stale), and a written `<base>` made relative links resolve differently from the deployed site. The iframe now goes to `/content/<set>/<page>?q#h` — served by the session server in production, proxied by Vite in dev, answered by `public/player-content-sw.js` in the player — and the parent adds its marked scripts on every `load` (`modes/webcraft/viewer/page-document.ts::instrumentPage`), so the page's own reloads and navigations are instrumented too. A directory URL (`./`, `sub/`) must serve its `index.html` on all three paths, as a static host does. Flag the instrumented document with its own attribute: marking `<html>` with the node mark made every element match `closest(mark)`, and edit mode, which skips the viewer's own nodes, made nothing editable.

- **Tie what a preview page reports to the document that reported it, not to the page that is current when it arrives** (2026-09-23 final review, webcraft). Text edits were queued in one array and saved into whichever page was current when the 800 ms debounce fired, so an edit on page A followed by a tab switch and an edit on page B wrote A's edit into B. Each loaded document now gets an id (`data-pneuma-instrumented="<id>"`), remembered with its content set, page, source path and the source text it was shown from; edits carry the id and are queued and saved per document, and flushed when the next page loads. Do not use `e.source === iframe.contentWindow` to accept them: an edit finished by the blur of a page switch arrives after the iframe has started the next navigation and no longer compares equal (measured in Chrome). A save is refused when the file changed after its document was shown (source-version precondition).

- **A text-edit handle must be verifiable against the source; a position on screen is not** (2026-09-23 reviews, webcraft). Tag + inner HTML + "n-th look-alike on screen" saved into `#first` when the page's script had removed `#first` and the user edited `#second`; a structural path with equal look-alike counts then saved Pro's paragraph into Basic's once a script moved the Pro card first — equal counts do not prove order. `describeEdit` (`modes/webcraft/viewer/source-edit.ts`, embedded into the page by `toString()`, so it must stay self-contained) sends the element's `id`, or an identity chain from the element up to its nearest ancestor with an id: tag, attributes and sibling tag+text at every level. The source must hold the same number of look-alikes, and exactly one must fit — by attributes (`data-product` on an ancestor), else by sibling text as a set (the heading travels with its card). Position (sibling order) counts only on a page with no `<script>` and no `on…` handlers, whose DOM cannot have moved: two identical cards swapped by a script look exactly like two that were not, so there the edit is refused. Test by building the handle on a DOM the "script" has changed, not by writing handles by hand. A refused edit or rejected write is shown in the preview ("Edit not saved", with Copy text / Retry save / Discard edit and show saved version; "Copied" only after `clipboard.writeText` resolves, otherwise the draft is selected for a manual copy) — the activity log only reaches the agent — and success is recorded only after the write resolves.

- **Changes that arrive while the preview iframe is loading must be kept, not dropped** (same review): a CSS change during a slow image load was never shown. `reloadIfShown` records them with the time they were seen; on `load`, `staleAfterLoad` compares that with when the document requested each file (Navigation/Resource Timing `startTime` + `timeOrigin`) and reloads once if any may be old. A directory URL (`sub/?q`) is the same page as `sub/index.html`: compare pages with `pageFromUrl`, never pathnames, or the viewer re-navigates and drops the query. A page the iframe reaches that is not an HTML page of the content set (a 404, another set, the app) is left uninstrumented, with a notice and a way back; nested undeclared pages of the set are tracked like declared ones.
