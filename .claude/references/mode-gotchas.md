...
- **盲测的前端不能由正在编辑的工作树的 Vite dev server 提供**(lucid 0.1.0 零泄题盲测,2026-09-16):盲测跑到第三轮时,子代理在同一工作树改 `modes/lucid/viewer/**`,HMR 把盲测页面重挂载、场景 iframe 重载、桥的 `registered` 短暂变 false、fps 样本混进初始化停顿,日志里是一串 Browser connected/disconnected,agent 自己报告"预览连接暂时断开"。改后端、脚本的仓库副本不影响正在跑的进程(session 装的是副本),`src/**` 与 `modes/*/viewer/**` 会。**盲测期间不动前端;或 `bun run build` 后用生产模式起盲测;或从另一个 worktree 起。** 配套:共享的 chrome-devtools 浏览器 profile 被另一个 Chrome 占着时,别去杀它——把会话 URL `open` 到用户自己的浏览器当 viewer,任务书走 `ws://…/ws/browser/<id>` 的 `session_subscribe` + `user_message` 投递(`scratchpad/send-brief.ts` 的形状),盲测就完全不碰共享守护进程。
...
- **iframe 里跑 WebGL 的 mode,截图和"场景 ready"是两件事;reload 之后立刻 capture 会拿到一张不该评的图**(lucid 盲测,2026-09-16):agent 把 `get-scene-state` 和 `capture` 批在一条命令里,reload 后桥报 `registered:false` 而 capture 照样出图。修在 viewer:`captureViewport` 先轮询桥直到 `registered && ready`(250 ms 一次,上限 4 s)再截,并把 `lastCapture{ready,registered,waitedMs}` 放进 `get-scene-state`;skill 规定 `lastCapture.ready===false` 的图不许送评。同一轮的两条:**桥的 fps 必须数真实的 `renderer.render` 调用**——自己起 rAF 计数量到的是调度节奏,一个 on-demand 渲染的场景会报 60 fps 却一帧没画(`fpsSource: "render" | "raf"` 说清是哪种);**评审 brief 走文件不走粘贴**——手抄进子代理 prompt 时 JSON 键被复制了一次,`judge-prompt` 现在写 `rounds/NN/judge-brief.md`、评审写 `rounds/NN/verdict.json`、`verdict` 默认读该路径。

- **Blender headless 的两个假信号**(lucid 0.1.0,2026-09-16 实测,Blender 5.2.1):(1) `hasattr(bpy.ops.wm, "anything")` **永远为 True**——`bpy.ops` 是惰性解析的,探测出来"存在"的算子一调用就 `could not be found`;要判一个算子在不在,查 `bpy.types.<MODULE>_OT_<name>`(5.2.1 只注册了经典的 `IMPORT_SCENE_OT_fbx/gltf`、`EXPORT_SCENE_OT_fbx/gltf`,传闻中的 `wm.fbx_import` 不存在)。(2) **Blender 在 `--background` 下可以写出一个几百字节的空 PNG 然后退出码 0**;渲染脚本的成功判据只能是产物文件大小(`blender.mjs render-views` 用 > 10 KB),不能是退出码。同一条的边界:`--background` 没有 OpenGL 上下文,Workbench/EEVEE 能渲,视口叠加层一律拿不到。

- **`gltf-transform optimize --simplify-ratio` 默认根本减不动**(同一轮实测):`optimize` 的 `--simplify-error` 默认 0.0001,减面器在远没到 ratio 之前就被误差上界卡停——`--simplify-ratio 0.3` 对 1152 面的网格删了 **0** 个三角形,而且退出码 0、日志正常。`glb.mjs optimize` 因此把 `--simplify-error` 默认成 1(与独立的 `simplify` 子命令一致),同一次运行 1152 → 345。判据:任何"减面后面数没变"的报告,先看误差上界,别怀疑几何。顺带:`optimize` 默认 `--prune-solid-textures`,纯色贴图会被整张删掉,`largestTexture` 从 `4096x4096` 变成 `none` 是真实产物不是 bug。

- **同源 iframe 里的 WebGL 画布,snapdom 截出来是黑的**(lucid 0.1.0,2026-09-16):没有 `preserveDrawingBuffer` 时,DOM 序列化拿到的画布是空的;可靠的截图只能由场景自己在同一个任务里 `renderer.render()` 后立刻 `toDataURL()`。所以 `captureViewer` 现在对整屏截图优先调 mode 自带的 `captureViewport`,再退回 snapdom / Electron;lucid 的 viewer 通过 postMessage 让场景页(`lucid-bridge.js`)渲一帧回传。任何"iframe 里跑 WebGL"的 mode 都要走这条路,不要指望 snapdom。
