...
- **Updates are silent until they land.** `desktop/src/main/updater.ts` owns the whole strategy: a check 15 s after ready, then every 4 h, plus on `powerMonitor` `resume` when the last check is older than 10 min; every scheduled check passes `shouldCheck()` and the `isPlatformAssetReady()` gate first. `autoDownload` pulls the installer in the background with no dialog and no progress UI. Only `update-downloaded` talks: dock badge (`app.setBadgeCount(1)`; Windows gets `setOverlayIcon` on every window, including ones created later), tray `↑` with a first menu item `Restart to Update to v…`, and one notification per version — all three call `restartToInstall()` → `quitAndInstall`. Only "Check for Updates…" still uses dialogs, because the user asked. The schedule gate, the state table and the tray strings are pure in `src/main/update-state.ts` and pinned by `desktop/__tests__/update-state.test.ts` (in `bun run test`; `bunfig.toml` ignores only `desktop/node_modules|dist-electron|release`). **e2e without packaging**: pin `desktop/package.json` to an older version and run `npm run build:main && PNEUMA_DEV_UPDATE=1 npx electron .` from `desktop/` — that flag is the only thing that sets `forceDevUpdateConfig`, which makes electron-updater read `desktop/dev-app-update.yml` (github/pandazki/pneuma-skills). The install step itself cannot be verified this way — an unpackaged dev app is unsigned.
