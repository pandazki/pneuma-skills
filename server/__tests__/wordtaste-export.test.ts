/**
 * Wordtaste Markdown export route.
 *
 * Wordtaste's draft.md is already pure body-only markdown (the skill keeps the
...
beforeEach(async () => {
  workspace = await mkdtemp(join(tmpdir(), "pneuma-wordtaste-export-"));
  app = new Hono();
...
describe("GET /export/wordtaste/download", () => {
  it("serves the active content-set's draft.md body as markdown", async () => {
    await seedDraft("from-idea", SAMPLE);
    const res = await app.request("/export/wordtaste/download?contentSet=from-idea");
  ...
  it("ASCII-slugging falls back to the content-set name for a CJK-only title", async () => {
    ...
    const res = await app.request("/export/wordtaste/download?contentSet=my-essay");
    const cd = res.headers.get("content-disposition") ?? "";
  ...
  it("exports the requested content-set when several exist", async () => {
    ...
    const res = await app.request("/export/wordtaste/download?contentSet=from-draft");
    const text = await res.text();
  ...
  it("auto-discovers the lone content-set when none is requested", async () => {
    ...
    const res = await app.request("/export/wordtaste/download");
    expect(res.status).toBe(200);
    const text = await res.text();
    expect(text).toBe(SAMPLE);
  });
  ...
  it("rejects a content-set that escapes the workspace (path traversal)", async () => {
    const res = await app.request(
      `/export/wordtaste/download?contentSet=${encodeURIComponent("../../etc")}`,
    );
