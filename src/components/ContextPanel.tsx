...
function SessionStatsSection() {
  ...
  // Both per-turn cost and lines-added/removed ride in claude-code's `result`
  // message envelope (no separate transport for either). Backends without
  // `costTracking` leave both at the default zero, so we gate the whole
  // result-stats group on a single flag — `showResultStats` — to make the
  // conflation explicit. If a future backend ships line-stats but no cost
  // (or vice versa), split this into two capabilities at that time.
  const showResultStats = session.agent_capabilities?.costTracking ?? false;
...
function TaskStatusIcon({ status }: { status: TaskItem["status"] }) {
  switch (status) {
    ...
    case "in_progress":
      return (
        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" className="w-3.5 h-3.5 text-cc-primary shrink-0 animate-spin">
          <circle cx="8" cy="8" r="6" strokeWidth="1.5" strokeDasharray="28 10" />
        </svg>
      );
    default:
      return (
        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" className="w-3.5 h-3.5 text-cc-muted shrink-0">
      ...
      <ul className="space-y-1">
        {servers.map((srv) => (
          <li key={srv.name} className="flex items-center gap-2 text-xs">
            ...
            <span className="text-cc-fg truncate">{srv.name}</span>
            <span className="text-cc-muted/50 ml-auto shrink-0">{srv.status}</span>
          </li>
        ))}
      </ul>
    </section>
  ...
  return (
    <section className="space-y-2">
      ...
      <ul className="space-y-0.5">
        {shown.map((tool) => (
          <li key={tool} className="text-xs text-cc-muted truncate pl-1">
            {tool}
          </li>
        ))}
      </ul>
  ...
  return (
    <section className="space-y-2">
      ...
      <div className="flex items-center gap-2 text-xs">
        ...
        <span className="text-cc-fg font-mono">{gitInfo.branch}</span>
