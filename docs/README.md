...
1. **[README.md](../README.md)** — 项目是什么、怎么安装、怎么用（面向人类，英文版默认；[中文版](../README.zh.md)）
2. **[CLAUDE.md](../CLAUDE.md)** — Agent 项目指引（Claude Code 读取）
3. **[AGENTS.md](../AGENTS.md)** — Agent 项目指引（Codex / 其他 coding agent 读取）

`CLAUDE.md` 和 `AGENTS.md` 是同一角色（agent 项目指引），分别给不同 backend 的 agent 读。三个根文件是 Pneuma 的 **single source of truth**，始终与代码同步。`docs/` 下的所有内容都是补充材料。
...
**关键原则：**
- 根文件（README / CLAUDE.md / AGENTS.md）是活文档，每次发版同步更新
...
| [controlled-state-surface.md](reference/controlled-state-surface.md) | 3.x Pneuma 受控状态全景（global / project / session 三层） |

### adr/

→ [ADR 索引](adr/README.md) — 14 个架构决策记录（ADR-001 ~ 014）
...
- [2026-04-30-project-onboard.md](archive/proposals/2026-04-30-project-onboard.md) — Fresh-project Discovery Report 模式设计（已实现，PR #99）
