...
- `ViewerLocator` — Agent 消息中嵌入的可点击定位卡片（`{ label, address }`，用户点击后经 ⑤ 触发 Viewer 导航到 `address`）
...
- `ViewerLocator` = `{ label, address }` + `navigateRequest` — 轻量定位（Agent 在消息中嵌入定位卡片，用户点击后 Viewer 导航到 `address`）。`address` 是一个 `ViewerAddress`（见下方一节）
- `capture` action 的 `params.address` — 同样是一个 `ViewerAddress`

**框架级内建 action：`capture`。** 每个 mode 的 viewer 都自带 `capture` action，无需在 manifest 里声明。Agent 通过它请求一张实时渲染的 PNG 截图——可选传 `params.address`（一个 `ViewerAddress`，见下方 ViewerAddress 一节）来只截某个对象：不传 `address` 截整个 viewport，传一个 coarse 半（`page` / `slide` / `file`）会先 navigate-then-shoot 把 viewer 切过去再截，传 fine 半（`selector` / `anchor`）则在当前画面里就地解析。runtime 把截图写入磁盘，返回一个文件路径，Agent 用 Read 工具直接查看。这让视觉自查（"渲染对不对、有没有溢出"）留在 Pneuma viewer 内部完成，而不是另起一个外部浏览器——外部浏览器渲染的是脱离 viewer 渲染规则的原始文件，看到的不是用户看到的画面。

`capture` 接收的对象就是用户选中元素时 ⑥ 上报的同一个 `address`——所以 select → 拿到 address → `capture` 那个 address 是一个闭环（webcraft 已验证）。`params.address` 也接受裸 `selector` 字符串以兼容旧用法。
...
- **上下文增强:** Runtime 调用 `extractContext(selection, files)` 生成 `<viewer-context>` 块，自动注入到用户消息前缀。Agent 借此理解 "这个按钮"、"这里" 等指代。当存在真实选择时，块里带一行机器可读的 `Address:` —— 用户选中对象的 `ViewerAddress`（JSON），Agent 可逐字复制回 `capture` 或一张 `<viewer-locator>` 卡片。
...
**数据契约:**
- `ViewerSelectionContext` — 选中元素信息。`address?: ViewerAddress` 是机器可路由的回指句柄（Agent 可直接喂给 `capture` 或 `<viewer-locator>`）；`selector` / `tag` / `classes` / `label` / `nearbyText` / `accessibility` / `thumbnail` 等是与之并存的人/Agent 可读描述符，不是 `address` 的别名
...
⑤ 和 ⑥ 这两个方向都在反复问同一个问题：**「viewer 里的哪个对象？」**——Agent 要给用户指一个对象（locator 卡片）、要截一个对象（`capture`）、要把 viewer 导航过去；Viewer 要回报用户选中了哪个对象（selection）、要回报当前位置（`<viewer-context>`）。`ViewerAddress` 把这个名词收敛为协议层唯一的一个类型。
...
**它是什么。** 一个 mode 定义、可序列化的「viewer 内对象引用」。对框架**不透明**——只有拥有它的 mode viewer 才知道 `{slide: 3}` 或 `{page: "about.html", anchor: "#pricing"}` 是什么意思。协议只固定「这个槽存在、哪些动词消费/产出它」；keys 和粒度由 mode 自己拥有，记在各自的 SKILL.md 里。一个非强制的约定帮 Agent 泛化：一个 address 把一个粗粒度的「where」（`page` / `slide` / `file` / `contentSet`）和一个可选的细粒度「within」（`anchor` / `selector` / `nodeId` / `lineRange`）配对；只需要粗粒度的 mode 直接省略其余部分。其中 `contentSet` 是唯一一个框架级保留键（由 store 自己解析切换 active content set），其余键全部 mode-opaque。

**为什么存在。** 在这个契约之前，同一个名词被 `ViewerLocator.data`、`capture` 的 `selector`、`ViewerSelectionContext` 的 `selector`/`file`、`navigate-to` 的 params 各自发明了一种 shape。代价落在 Agent 身上：每个 feature 一套寻址词表，且互不通用——用户选中一个元素拿回 `selector`，Agent 想截它或指回它，`capture` 和 `ViewerLocator` 却要不同的形状，select → view → point 这个最自然的 QA 闭环根本无法表达。`ViewerAddress` 让**交互模式不变**（「拿到一个 address，然后 point / view / navigate 它」），只有**address 内容**随 mode 变。这与 `fileRef`（`BackendModule.toolFileRef`）是同一个先例：一个跨切面、运行时归一化的引用类型。

**一个名词，每个动词。**

- Viewer **产出**（⑥）：selection 回报 `{ address, ...descriptors }`；`<viewer-context>` 在 `Address:` 行带上当前选择的 address。
- Viewer **消费**（⑤）：`ViewerLocator = { label, address }` 给用户指；`capture(address)` 给 Agent 截；navigation（`navigateRequest`）把 viewer 移过去。

因为同一个 `address` 既被产出又被消费，闭环成立：用户选中对象 → Agent 收到它的 `address` → Agent `capture` 那个 address，或发一张指回它的 `ViewerLocator`。

**每个 mode 自己定义词表。** webcraft 是已验证的参考实现（`{ contentSet?, page, anchor?, selector? }`，深到 DOM region）；slide 只需停在 slide 级（`{ contentSet?, slide }`）；doc 用 heading / lineRange；diagram 用 nodeId；draw 用 elementId。这些粒度互不约束——每个 mode 在自己的 SKILL.md 里记录自己的 address 词表（每个 key 标 *coarse* 或 *fine* + 一行含义），框架不持有任何 mode 的 keys。完整设计见 [`docs/design/2026-05-20-viewer-address-contract.md`](../design/2026-05-20-viewer-address-contract.md)。

`<viewer-locator>` 聊天标签的规范属性是 `address='{…json…}'`；解析器仍接受旧的 `data='{…}'`，以便契约落地前写入的历史会话里的 locator 卡片仍能渲染。新的 Agent 输出一律用 `address=`。
...
- **skill-installer**: manifest → CLAUDE.md（注入 skill prompt + action descriptions + viewer API + proxy docs）。viewer API 段是一个**纯路由器**——它命名各个通道，把 `ViewerAddress` 的具体词表指向 mode 自己的 SKILL.md，框架不持有任何 mode 的 address keys
