...
|---|---:|---|---|
| `claude-code` | `true` | CLI streaming input (`--input-format stream-json`) | Write another SDK `user` envelope while the agent loop is running; Claude consumes it at the next interruptible point. The input frame carries no turn id, so a turn-boundary race is guarded only by the bridge's pre-write check — see invariant 5. |
...
```ts
{
  ...
  success: boolean;
  error?: string;
  reason?: "unsupported" | "no-active-turn" | "transport-error";
}
```

`error` is the human sentence; `reason` names the cause. The two causes Pneuma
decides for itself — the transport cannot steer, or there is no turn to steer
into — are re-worded by the browser in the reader's own language, so a session
running in Japanese does not get a Chinese-framed English sentence. Anything the
transport itself threw is `transport-error`, and that text is shown verbatim
because it never belonged to Pneuma to translate.
...
5. A backend/protocol race must fail explicitly via `steer_result`; it must
   not silently fall back to interrupt-plus-resend or to a new turn. How
   strong that guarantee is depends on what the transport can express. Codex
   names the turn it is aiming at (`expectedTurnId`), so the app-server itself
   refuses a steer meant for a turn that has already ended. Claude Code's
   streaming input carries no turn id, so the bridge can only check that the
   session is mid-turn immediately before it writes. That check covers the
   window that actually matters — everything from the click through viewer
   context enrichment and the trip to the server — but if the turn's `result`
   lands between the check and the write, the guidance opens a new turn
   instead of being refused. Nothing is lost when that happens: the message
   still reaches the agent, and the session's next `status_change` re-syncs
   the UI.
