...
Architecture and runtime reference for Pneuma Skills. Start with [AGENTS.md](../../AGENTS.md) for development rules and skill entry points. Update this document alongside contract changes; it is the canonical contract directory.
...
| **ModeCatalog** + `ModeCatalogEntry` / `ModeArchiveRef` / `ModeInstallRecord` / `ModePackageStamp` / `ModeInstallState` | `core/types/mode-catalog.ts` | `scripts/publish-modes.ts` 每次 release 生成 `modes/catalog.json`(不手写、不入库):`modes/distribution.json` 没 bundle 的每个 mode 都经 `snapshot/mode-build.ts` 就地构建、打上 `pneuma-package.json` stamp、去掉 `__tests__/` `harness/` `showcase/` 打包、传到 `official/v<core>/<name>-<modeVersion>.tar.gz`,并用 size + SHA-256 钉死;`displayName` / `description` / `icon` 由 `core/utils/manifest-parser.ts::parseManifestTs` 从 manifest 抄出 | `core/mode-catalog.ts`(读 catalog、装/校验/记录,判断 `installed` / `stale` / `not-installed`)→ `core/mode-resolver.ts`(`catalog` 源)、`core/local-modes.ts`(`installed` 标记)、`server/catalog-routes.ts`(`/api/catalog*`)、launcher 卡片;`scripts/verify-mode-catalog.ts` 是 release job 建 tag 前的闸门(先拿这个 checkout 的 catalog mode 全集与 catalog 对账——缺、多、或条目版本与树里 manifest 不符都算失败——再 HEAD 每个钉住的 archive);`package.json` 的 `files` 与 `desktop/electron-builder.yml` 只装 bundled 集合加 catalog(同样从 `distribution.json` 派生,由 `scripts/__tests__/release-packaging.test.ts` 守住)。不变量钉在 `core/__tests__/mode-distribution.test.ts` |
| **Mode viewer host ABI** — `HOST_ABI_VENDOR_URLS` / `HOST_ABI_EXTERNALS` / `HOST_ABI_VENDOR_SHIMS` + `hostAbiImportMap()` / `resolveHostAbiExternal()` | `snapshot/mode-build.ts` | 一张数据表:外部 mode viewer 唯一可以向宿主借的东西 —— React、ReactDOM、两个 JSX runtime、宿主 Zustand store、i18next 与 react-i18next。externals 由 URL 表派生,shim 源码与它同表,所以「标成 external 的」「importmap 里写的」「server 真 serve 的」三者不会漂。React / react-dom(含 react-dom/client)/ 两个 JSX runtime / react-i18next 的 shim 导出名**从宿主实际安装的包读出**,不手写;`hostAbiExportNames()` 给出每个 specifier 可 import 的名字,`findUnresolvedHostAbiImports()` 据此检查产物,`buildModeViewer` 遇到 shim 不提供的名字直接构建失败 | `snapshot/mode-build.ts::buildModeViewer` 是**唯一** builder(`bin/pneuma.ts` 启动时构建外部 mode、`scripts/publish-modes.ts` 发布 catalog mode、`snapshot/mode-publish.ts` 与 `server/mode-maker-routes.ts` 的 publish 路径都调它);`server/index.ts` 用 `HOST_ABI_VENDOR_SHIMS` serve `/vendor/*`、用 `hostAbiImportMap()` 往 index.html 注入 importmap;`src/main.tsx` 在加载任何 mode bundle 之前挂好 `window.__PNEUMA_*`。Bun 1.4.0 的两个坑与不变量钉在 `snapshot/__tests__/mode-host-abi.test.ts` |
...
| **ViewerSelectionContext** + `extractContext()` | `core/types/viewer-contract.ts` | Each viewer implements `extractContext` | `src/ws.ts` prefixes every ordinary `user_message` with the `<viewer-context>` block (and `<user-actions>`), and `server/ws-bridge.ts::prepareIncomingUserMessage` folds queued `<pneuma:env>` tags in front. A slash command (`core/utils/slash-command.ts::isSlashCommandMessage`) is exempt on both layers and travels verbatim — backends resolve commands by the first characters of the turn, and a command turn never reaches the model as prose; the held-back context rides the next ordinary message |
...
| **Reasoning effort + session token usage** — `ModeManifest.agent.reasoningEffort` → `AgentLaunchOptions.reasoningEffort` → codex `turn/start.effort` (`thread/start` has no such field); `SessionState.token_usage` (cumulative input / cached / output / reasoning, raw and unpriced) on `session_update`, filled by the Codex adapter from `thread/tokenUsage/updated.total` | `core/types/mode-manifest.ts`, `core/types/agent-backend.ts`, `server/session-types.ts` | `bin/pneuma.ts` passes the manifest's effort at launch; `backends/codex/codex-adapter.ts` sends it per turn and reports usage | `src/components/ContextPanel.tsx` shows the tokens; a mode prices them itself (`modes/lucid/skill/scripts/costs.mjs` → the cost panel) |
| **SubagentInfo** + `subagent_update` + `BackendModule.subagentSpawn`(归属键是 envelope 上的 `parent_tool_use_id` = 派出该 agent 的 `tool_use.id`,主 agent 为 `null`) | `server/session-types.ts`(roster 快照 + browser envelope)、`core/types/agent-backend.ts`(`SubagentSpawnRef` + 可选方法) | Codex 侧 `backends/codex/codex-adapter.ts` 按 `threadId` 认线程、自己发 roster;Claude 侧 `server/ws-bridge.ts` 用 `subagentSpawn` 认出 `Task`/`Agent` 调用并以那个块的 id 建条目。两条 bridge 都先压进 `messageHistory` 再广播 | `src/store/subagent-slice.ts`(快照整条替换 + 缺条目兜底派生);`src/components/ChatPanel.tsx` / `SubagentCard.tsx` 渲染卡片、strip 与 agent view;`src/replay-engine.ts` 折叠回放。子代理永不结束主 turn(无 `result` / 无主 `status_change` / 不动主上下文仪表) |
...
| **catalog**(第一方、不进包) | `backlot`、`doc`、`sprite` …(和 builtin 一样是裸名字) | `~/.pneuma/catalog/<name>/` |
...
安装顺序是契约:**下载 → 校验 size 与 SHA-256 → 解压到临时同级目录 → 原子 rename → 最后写 `.pneuma-install.json`**。记录写在最后,所以"有目录没记录"= 未完成,直接删掉重取;失败(404 / 校验不符 / 中断 / 解压失败 / `unsafe-archive` / `core-version-mismatch` / `record-write-failed`)各有独立 `ModeInstallError.code`,且不留半个安装、绝不回退到别的核心版本构建的旧包。**archive 是不可信输入**:解压前先 `tar tzf` 列表,拒绝绝对路径、`..` 和叫 `.pneuma-install.json` 的成员;解压后遍历暂存树,拒绝一切非普通文件/目录(symlink、hard link、设备节点);记录用 `wx` 创建而不是写入,所以谁都无法把它重定向到安装根之外。stamp 的 `coreVersion` 与 catalog 的比对,是「这批字节确实是这个 release 造的」唯一的现场证据。`~/.pneuma/catalog/` 与 `~/.pneuma/modes/`(用户装的、evolve 出来的)彼此独立,不会在 launcher 里出现两次。
...
完整的"Pneuma 在磁盘上管了什么"图谱见 [`docs/reference/controlled-state-surface.md`](controlled-state-surface.md)。下面只列契约层关键点。
...
**Session-scoped slash commands**:`BackendModule` 多了一个可选的 `commandsDir`。Claude Code 把 `<cwd>/.claude/commands/*.md` 当原生 `slash_commands` 在 `system.init` stream-json 事件里上报,因此 installer 会把 `templates/session-commands/{borrow,handoff}.md` 复制到 `<installTarget>/.claude/commands/`——它们在 in-session chat 输入框里以 `/borrow` 与 `/handoff` 出现(quick + project session 都装)。两条都放在这里而非只写进 `pneuma-project` skill,是因为那个 skill 只在 project session 装,quick session 的 agent 否则无从得知这两个原语存在——机制挂着却没人能触发。Codex 把它的 *skills*(非 project command 文件)映射成 slash_commands、Kimi(ACP `available_commands_update` 上报的是它自己的命令与 skills,不读 project command 文件),所以两者 `commandsDir` 留空、installer 这一步直接跳过。**Gate 在 `commandsDir` 字段而非 backend 条件判断**——没有 `if (backendType === ...)`。
这一步由 `installSessionCommands(installTarget, backendType)` 单独承担,`installSkill()` 调它,`bin/pneuma.ts` 在**跳过 skill 安装的那条腿上也调它**(resume 时选「保留当前技能版本」、replay、viewing)。两条命令随 pneuma 发版(源在 `templates/`),不随 mode skill 发版,所以「这个 session 有没有 `/handoff`」不能由一个关于 *mode skill* 的决定来定——3.43.0 就是这样让一个 resume 过的 session 挂着交接机制却没有入口。紧邻的 workflow scripts **不能照抄这个提升**:它们来自 `modeSourceDir`、随 mode skill 发版,拒绝技能更新时保留旧脚本才是对的。
...
每个 session 注入:`PNEUMA_SESSION_DIR`(agent CWD;当前 backend 的 skill 目录、`CLAUDE.md` 或 `AGENTS.md`、state 文件都在这)、`PNEUMA_HOME_ROOT`(project session 为 project root,quick 为 workspace)、`PNEUMA_SESSION_ID`。项目 session 额外注入 `PNEUMA_PROJECT_ROOT`。
...
- **Confirm**:server 原子写 `<targetSessionDir>/.pneuma/inbound-handoff.json`、best-effort kill 源 backend、记录 `switched_out` / `switched_in` 事件、spawn target。**源是 quick session 时走另一条腿**(3.43.0):`resolveSource` 返回 `{ kind: "quick", workspace, backendType }`,confirm 把这个 workspace 交给一个新的 quick session——不写 `project.json`、不迁移旧状态、不记 `switched_out`(那份 history 马上被新会话覆盖)、不预先 mint session id(quick 的 id 由会话自己在启动时生成),唯一痕迹是新 `session.json` 里一个没人追的 `sourceSessionId`。启动侧由 `startsOverPersistedSession` 判定:带着 staged inbound 启动的 quick session 不 resume 上一个 mode 的对话。**需要互相共享的两个会话是 project 的职责,别把 project 语义塞进 quick。**Target 的 skill installer 把 inbound JSON 装到 `pneuma:handoff` block;target agent 第一轮读完并 `rm`。
- **Cancel**:server 派一条 `<pneuma:handoff-cancelled reason="..." />` synthetic user message 回源 agent。

完整设计见 [`docs/archive/proposals/2026-04-28-handoff-tool-call.md`](../archive/proposals/2026-04-28-handoff-tool-call.md);项目层全貌见 [`docs/archive/proposals/2026-04-27-pneuma-projects-design.md`](../archive/proposals/2026-04-27-pneuma-projects-design.md)。
