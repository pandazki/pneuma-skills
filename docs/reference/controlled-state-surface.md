...
| `.kimi-code/skills/<installName>/` | Kimi 后端的 skill 安装目录 | Skill installer | 同上 |
| `CLAUDE.md` | Claude 后端的指令装配台（见下） | Skill installer | 启动时拼装；启动后由部分子系统增量更新 |
| `AGENTS.md` | Codex 与 Kimi 后端的指令装配台 | Skill installer | 同上 |
...
每次启动时，`server/skill-installer.ts` 把 mode 的 skill 目录 copy 到 `<sessionDir>/<backend-specific>/skills/<installName>/`（Claude `.claude/`、Codex `.agents/`、Kimi `.kimi-code/`——具体由 `BackendModule` 的 `skillsDir` 决定），然后在 `CLAUDE.md` / `AGENTS.md` 上拼装一组**命名 marker block**——每个 block 由不同子系统负责填充，互相正交。
