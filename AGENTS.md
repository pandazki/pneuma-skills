...
- **Editor-open 策略**（`server/editor-bridge.ts`）：`/api/system/open-in-editor` 对**文件**走 CLI-first（bundled `cursor`/`code` 启动器），对**目录（项目根）走 `open -a`**。四条铁律：
  0. **目录目标必须用 `open -a <App> <dir>`，不走 CLI**：bundled `cursor <folder>` 在本地模式下把文件夹交给已运行实例的 `cli.js` IPC 打开，最新版 Cursor 在这条路径上崩 AgentPanel（`Cannot read properties of undefined (reading 'trim')`）——而 Antigravity 等无此 panel 的 fork 不受影响。`open -a` 走 LaunchServices open-folder Apple event（Finder 同款路径），可靠且绕开崩溃。这是 CLI-first 之前的原始行为；CLI-first 只为**文件**保留（要 `--goto` + 项目窗口聚焦 + reveal，`open -a` 表达不了）。`openInEditor` 里 `statSync(absPath).isDirectory()` 分派。
