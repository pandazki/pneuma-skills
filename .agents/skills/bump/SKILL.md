---
name: bump
description: Release Pneuma Skills when the user requests a version bump or release. Updates versions and release notes, validates, pushes main, and verifies publishing.
---
...
Read `references/release-process.md` before executing the steps; it owns the detailed release checklist and recovery notes. Paths in this procedure are repository-relative unless marked as skill-relative.

## Steps

### 1. Sync main

Inspect the branch, status, and diff first. Preserve unrelated work. Include the
current task's authorized changes in the release; do not ask the user to resolve
your own pending edits. Fetch origin and compare the intended release with
`origin/main`. Fast-forward when possible; use an isolated checkout if unrelated
edits prevent a safe update. Resolve routine integration issues within scope and
ask only when including or changing unrelated work requires a decision.
...
Review `README.md`, `README.zh.md`, `AGENTS.md`, and `docs/reference/project-guide.md` against the current codebase state. Fix any contradictions, outdated info, or missing features. This is a **holistic review**, not just appending — trim stale content, update tables, fix version numbers.
...
rg -n '"1\.3\.0"' core server backends modes --glob '*.ts' --glob '*.tsx'
```
...
Update all four files in a single commit:
1. `package.json` — `"version": "X.Y.Z"`
2. `desktop/package.json` — the same version
3. `AGENTS.md` — `**Version:** X.Y.Z`
4. `CHANGELOG.md` — new version section (from step 3)
...
Before pushing, run typecheck, the **full** test suite, and the production build:
