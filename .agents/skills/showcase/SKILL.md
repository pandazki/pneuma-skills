...
name: showcase
description: >-
  Generate or refresh a Pneuma mode's launcher showcase: metadata, hero image, and feature images. Use when the user requests showcase materials.
---
...
Generate **4 images** (1 hero + 3 highlights) using the active harness's image-generation capability. Follow its image-generation skill when available; inspect reference images first. If image generation is unavailable, complete the metadata and report which assets still need generation.
...
3. Inspect the actual images for composition, readable text, dimensions, and intended visual style.
4. Compress PNGs with the release size guidance before committing. Commit the showcase changes when the active task includes committing; otherwise leave the reviewable files in the working tree.
