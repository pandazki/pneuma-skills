...
- **Illustrate's gallery shows the new models at work.** Four refreshed showcase images follow one creative brief through visual directions, a finished poster, and a real Flare edit. The edit preview shows the actual original and result; its copy describes reference-guided editing without promising pixel-perfect preservation.
...
### Added
- **Plotwise — a learning studio that shoots your custom course while you choose the plot.** Name something you want to learn. The agent plans a grounded outline — every beat carries its evidence: code-rendered figures, pinned citations, verification runs — while you settle the look on a style board (a preset, a recommendation, or your own description) by confirming a five-second sample shot about your topic. The course is then written as a screenplay, one scene per beat, each a run of 1–6 chained 5–15 s narrated shots on MiniMax H3 Max via fal.ai, and a play-manager process shoots it ahead of you like a visual novel: at the end of every scene you continue along the spine, take the one detour written for that scene, or ask your own question, which comes back as a grounded scene. What you do not choose is pruned and cancelled at the queue; no model runs on the click path. The video model is never trusted with facts — knowledge visuals reach it only as code-rendered references it must reproduce, and every shot's narration is transcribed back and checked word for word before you hear it. The stage holds an interlude between scenes (the last frame, a recap of what was just said, what comes next, shot progress), a rail of your main line, the course map, and an evidence panel per scene. Three blind trials on Codex at 480P: the opening ready about a minute after the manager starts, a 40 s scene 50–100 s after it is queued, choices in under 4 s, a question turned into a finished scene in about two minutes. Needs a fal.ai key and an OpenRouter key.
- **A shared fal.ai queue client (`modes/_shared/scripts/fal-queue.mjs`).** Every fal-backed script submits through the queue endpoint instead of holding a socket open for the whole render: status is polled, the job is cancelled remotely when the caller aborts, when its deadline passes or when its status can no longer be read, and a submit is retried only when the failure proves no job was created — fal has no idempotency key, so a request that left and lost its answer is reported rather than sent twice. `generate-video.mjs` moved onto it, and every clip it returns is loudness-normalized to −16 LUFS in one audio format, with out-of-range reference images letterboxed and downloads timed by idle rather than total length. `transcribe.mjs` joins the shared scripts.

### Fixed
- **Bun 1.3.14 is now the floor, and the reason is stated.** Bun ≤ 1.3.13 deadlocks the whole server on macOS arm64 under a churning workspace — main thread and file-watcher thread parked on one lock, every route and the WebSocket dead (oven-sh/bun#26762, fixed upstream by the mimalloc v3 upgrade). CI and the desktop bundle run 1.4.0; the too-old message says what breaks instead of "terminal features may not work".
- **`/content/*` Range responses no longer send the whole file.** On Bun 1.4.0 `.stream()` on a sliced `Bun.file()` yields every byte, and the CORS layer in front of the route re-wraps the body as a stream — so a 206 whose body contradicted its Content-Range reached the browser, which refused the media outright. The byte window is now a Node read stream, and a range past the end answers 416.
- **The file watcher ignores Python environments.** A grounding agent once built a 12,704-file virtualenv inside a workspace; the watcher swallowed the flood and went silent for the rest of the session, and the next server start spent minutes stat-ing the tree. `.venv`, `site-packages`, `__pycache__` and the usual caches are excluded, mirroring shadow-git.
- **The browser re-reads the workspace after a WebSocket reconnect.** Anything the watcher broadcast while the tab was away was gone for good; a `session_init` after the first hydration now fetches the file listing again, and an empty listing is applied like any other.

## [3.44.1] - 2026-09-01

### Fixed
- **ClipCraft's playhead snaps to clip boundaries, like every other timeline gesture.** Clip drag, edge resize and the split tool all snapped to nearby edges within 5px; the playhead — the cursor you move most — was the one gesture that didn't, so it landed on whatever sub-frame value the pixel produced. That is not only untidy: split-at-playhead lands exactly where the playhead is, so a 5.973s playhead left a 27ms sliver of a clip, nearly invisible on the timeline and enough to break first-/last-frame anchoring downstream. Ruler clicks and handle drags now snap to clip starts and ends across every track plus `t=0`; Shift bypasses the magnet for the frame you really meant, and can be pressed or released mid-drag. Programmatic seeks — Home / End, the transport, locator navigation, hover-scrub restore — stay exact and never snap.
- **Pressing Play at the end of a ClipCraft composition starts a new pass instead of doing nothing.** The playhead parked at the end left the transport inert: you had to click Skip-to-Start first. The cause was upstream — `@pneuma-craft/video`'s engine paused itself the moment its loop saw `time >= duration`, and its clock resumed from wherever it was paused, so `play()` ran for exactly one frame and re-paused. Reported as pandazki/pneuma-craft#3 and fixed in the engine, which now rewinds when it is at the end, in one clean `paused → playing` transition. Space behaves the same, because both go through the engine. Mid-composition Play still resumes in place.
...
### Improved
- **WebCraft's design intelligence now tracks Impeccable v4 — and the headline is deletion.** Upstream cut its always-loaded core to a fraction of its length, on the finding that current models follow short structured instruction better than long insistent prose, and that a heavy core turned "tighten this button" into a full product interview. The skill follows: the quality floor, the ban list and the AI Slop Test move into a reference the agent loads immediately before editing UI and not at all while planning, and every command upstream consolidated is far shorter here too. The brand/product register is retired for four **visitor modes** — Persuade, Operate, Read, Experience — decided per surface from what the visitor came to do, so a tool's landing page is still Persuade and a fashion house's docs are still Read. New work now names its job kind before anything else, because a blank slate, a page inside an existing product, a section added to a working page, a redesign and a scoped refinement each earn a different amount of freedom; getting that wrong is what restyles a product around one new section, or polishes the look you asked to be rid of. Every build writes a five-block statement of intent before code and ends on a reviewed verdict of rebuild, fix, or ship — judged from screenshots of the live preview by a reviewer that never saw the build thread. Pages remember their own strategy between sessions. All 22 commands keep their names, so open workspaces need no migration.
- **Kami tracks tw93/Kami V1.13.0.** All 18 diagram templates now name their own typeface instead of inheriting one, which is what keeps a diagram intact once you lift its `<svg>` into a document: a two-character Chinese label can no longer come back drawn in two different faces. Two brand tints that the design docs described but nothing declared are now real tokens — one of them was silently resolving to nothing in every diagram that used it — and the documented letter-spacing finally matches what the stylesheet has always shipped. Cards and code blocks drop their hairline borders in favour of fill, a new deck pre-flight asks the six questions worth settling before a slide gets drafted, and three upstream release principles land as working rules: name the page target and the checks before layout, keep a visual fix inside the area you were asked about, and stop hidden text or an unresolvable image from satisfying a check.
- **The logo & mascot playbook tracks its upstream again, and now records which commit.** It had none, so nobody could tell what it had been synced against. Composition relaxes to a dominant corner — the subject fills most of the square out of an assigned side, and a batch splits evenly between the two so every direction is seen from each. Depth is asked for in one sentence rather than a percentage, because a number invites the model to hit it. And generation prompts stop naming the deliverable: the words "logo", "app icon" and "opaque" pull image models toward badges, frames and cut-outs, so the prompt describes an image and the logo is what you make of it afterwards.
- **Cosmos borrows two ideas from Understand-Anything v2.9.0.** A projection's merge step now reports what each partition actually contributed, and a slice that comes back empty is re-dispatched once before being reported as a hole; a slice whose nodes were all claimed elsewhere is named as an overlap rather than looking like one that found nothing. Dropped edges leave with the id that went missing, which names a node the projection is short of. There is also a new node vocabulary for design systems — component, instance, token — carrying the relationships that projecting a component library as ordinary code flattens into "imports".

### Fixed
- **A dead partition can no longer vanish from a cosmos projection, and a skipped stage can no longer take the whole run down.** A subagent that dies resolves to nothing rather than failing, so a whole partition could disappear between extraction and merge without leaving a line behind — and four stages guarded that case with a fallback that never fired, which meant the next property access ended the run with a type error instead.
- **`/handoff` and `/borrow` survive a session resumed on its current skill version.** Both commands were installed as part of the mode skill, so every reason to skip that install — resuming with "keep current skill version", replay, view-only — took them down with it. A session resumed that way had the entire handoff machinery mounted and no way for its agent to reach it, which for a quick session meant no way to learn the two primitives existed at all. They ship with Pneuma, not with the mode skill, so they are now installed on that path too.
...
- **ELI5 explainers share and deploy.** A shared eli5 session now renders as a live read-only ladder in the hosted player instead of falling back to the local client, and the mode joins the export/deploy path webcraft and slide use: `/export/eli5` previews every rung, downloads one page self-contained or the whole topic as a ZIP, and deploys to Vercel or Cloudflare Pages in one click. The deployed artifact is the ladder as a small site — a landing page listing the rungs in order, and a JavaScript-free pill on every page that names the current level and expands to the rest, so no deployed explainer is a dead end.
...
### Added
- **A bansho lecture can carry a figure the board cannot draw itself.** Charts, graphs and the ink on the words are the board's own and are drawn a line at a time in front of the audience — but a neuron, a cross-section, a thing whose likeness *is* the meaning had no answer, so lectures that needed one went without. There are two tiers now, and both are decided in `plan.md` **before the first board step**: sayable with `chart` / `graph` / ink → tier 1; needs real hand-drawing ability → tier 2, ordered from an outside hand. Every tier-2 picture is ordered in ONE batch the moment the design is settled — never mid-lecture, because each is real money and the better part of a minute, and a wait that lands between two sentences is paid by the person watching. The agent carries nothing extra while writing, which was the whole requirement.
- **The skill owns the look; the agent fills in the subject only.** The prompt's opening is fixed and quoted verbatim — chalk on a blackboard, line art, no shading, no lettering — so a figure arrives in the board's own hand instead of whatever a generator felt like. **No text labels** is load-bearing twice over: generated lettering is unreliable, and a label baked into a picture is in the wrong hand, since every other word on that board was written by the pen. Labels go on the board, beside the picture.
- **One asset serves both themes.** Chalk is bright and slate is dark, so the picture's own luminance is the mask and the board paints its *own* ink through it — chalk on slate in the dark theme, ink on plaster in the light one, with nothing regenerated when the theme flips. The box is sized from a **declared** aspect in `illustrations/manifest.json`, never measured off the file, so the same lecture still folds identically at every window size.
- **The board's voice is ByteDance Seed-Speech, and the shared TTS CLI now carries two voices.** Seed-Speech reads a bilingual lecture the way a bilingual person does — 「阿姆达尔定律」 and "NVIDIA" in one breath with no language flag set at all — which is what a board asks for constantly. `gemini-3.1-flash-tts` remains the CLI default (clipcraft's voice does not move) and is one flag away for inline expressive tags or a WAV.
- **The narration is fused into one track, so no sentence ever starts cold.** Every clip used to be a fresh `src` on one shared `<audio>` element, which means every clip began at `readyState 0` with nothing buffered — measured on 4 of 4 starts. That is the hitch a listener hears as a swallowed first syllable. The clips are now mixed once, at authoring time, into a single continuous file: decoded to PCM, laid by **sample index** rather than by floating-point seconds, and encoded exactly once, because concatenating already-encoded MP3s accumulates each one's encoder delay until the voice drifts off the pen. One `src`, set once. The track is laid in **wall time**, not the board's canonical time — the canonical clock deliberately pins at a pen-down while a long clip finishes, and playback does not, so every earlier hold is added back in. The per-clip path stays as the fallback.
- **The board checks the track against itself instead of trusting it.** A single track is globally aligned: after the first second there is no per-step resync, so a layout computed against a stale document would let the voice and the pen part company in public and look like a scheduling bug. The track therefore ships an ordered hash-and-offset manifest, and the board recomputes the layout from the live document at load and refuses one that has moved — `staleTrack` names the reason and the board falls back rather than playing something out of step. Editing one sentence shifts every later position, so the same check is what catches a track that needs remixing.
- **The voice has an off switch.** A mute control sits beside the playback rate in the transport. It silences the output stage only — it never pauses the element, so the pacing, the schedule and where the pen is are untouched: a muted lecture is the same lecture. It works on both narration paths, survives a mid-lecture fallback, and is remembered per browser (whether sound plays is a fact about the room you are in, not about the lecture, so it deliberately stays out of the session and the preference files the agent reads). A browser that blocks autoplay still says so — but not while you are the one who asked for quiet.
- **MP3 length is measured, not estimated.** Seed-Speech returns no WAV at all, and a narration manifest cannot record a clip without its `seconds` — so the CLI now reads an MP3's real length from its frame header (a Xing/Info frame count where the stream carries one, otherwise the CBR bitrate over the audio bytes with ID3 tags excluded). Verified against real clips: identical to `afinfo` on every one, and to the browser's own decode.

### Fixed
- **An image in a lecture used to parse, hold its place and draw nothing.** `![…](…)` was a step the board silently skipped with an `unsupportedStep` badge — a hole in the middle of a talk. It is drawn now, participates in the room like everything else (fold, collision, `check-board`), and when it cannot be drawn it says which picture and why instead of leaving a gap.
- **Every figure was drawn at the full board width and hung off the edge.** A half-width column figure came out 1242px wide in a 1242px board — over half the picture outside it. A percentage width on an absolutely positioned box resolves against the containing block, not against the insets the mount had just written, so the region arithmetic reached the node and did nothing. The size now comes from the region's own rectangle through the declared aspect: fit, not fill, centred in its region, still pure arithmetic on canonical coordinates with nothing measured.
- **The picture probe re-ran on every keystroke.** The effect keyed on the manifest *object*, which the aggregate source mints fresh on every watched-file emit — one per `board.md` append while the agent live-writes — so a burst of typing fired a fetch per picture on the hot file-watch path. It keys on a fingerprint string now, the way the voice layer's probe already did.
- **A format or language a vendor cannot accept is refused before the request.** Asking a voice with no WAV for a `.wav` path used to write MPEG bytes into a file with the wrong name, after which the length could not be read and the manifest recorded a clip it could not pace — every step looking like it worked. A language spelled the other vendor's way cost a paid request and returned no audio. Both are caught locally now.
