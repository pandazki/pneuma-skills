/**
 * Palate Mode Manifest — Taste Writing Studio.
 *
...
const palateManifest: ModeManifest = {
  name: "palate",
  version: "0.1.0",
  displayName: {
    en: "Palate",
  ...
  description: {
    ...
    "zh-CN": "口味写作工作室 —— 跨模型家族协同，去掉文字的「一眼 AI」味，并在过程中习得你的声音",
    "zh-TW": "口味寫作工作室 —— 跨模型家族協同，去掉文字的「一眼 AI」味，並在過程中習得你的聲音",
  },
  changelog: {
    "0.1.0": [
  ...
  agent: {
    ...
    greeting: `<system-info pneuma-mode="Pneuma Palate" skill="pneuma-palate" session="new"></system-info>
The user opened Palate with a concrete writing goal, not a configuration task. First run scripts/cross_family_probe.sh to detect which model families are available (writes .pneuma/cross-family.json), then ask for their goal if it is not already given: entry A is an outline/idea to write from, entry B is a draft to de-AI. Greet in 1–2 sentences. Never frame this as taste configuration or a setup wizard — onboarding is a byproduct of "give me your writing goal."`,
  },

  init: {
    contentCheckPattern: "**/draft.md",
    seedFiles: {
      "modes/palate/seed/from-idea/": "from-idea/",
      "modes/palate/seed/from-draft/": "from-draft/",
      "modes/palate/seed/worked-example/": "worked-example/",
    },
    seeds: [
      {
        id: "from-idea",
        sourceKey: "modes/palate/seed/from-idea/",
        displayName: {
          en: "Start from an idea",
          "zh-CN": "从一个想法开始",
        },
        description: {
          en: "You have an outline but no prose. Palate writes the first cross-family draft, then de-AIs it with you.",
          "zh-CN": "你有提纲但没有正文。Palate 跨家族写出初稿，再和你一起去 AI 味。",
        },
        tags: ["Entry A", "Idea"],
      },
      {
        id: "from-draft",
        sourceKey: "modes/palate/seed/from-draft/",
        displayName: {
          en: "De-AI a draft",
          "zh-CN": "改一篇「一眼 AI」的稿子",
        },
        description: {
          en: "You have prose that reads one-glance-AI. Paste it in; Palate freezes the kernel and runs the disruption ladder.",
          "zh-CN": "你有一篇「一眼 AI」的稿子。贴进来，Palate 冻结内核、跑扰动档位。",
        },
        tags: ["Entry B", "De-AI"],
      },
      {
        id: "worked-example",
        sourceKey: "modes/palate/seed/worked-example/",
        displayName: {
  ...
  evolution: {
    directive: `Learn the user's writing voice and de-AI taste from session history. Maintain a concise, cross-mode summary of their voice signature (breathing/hedging habits, metaphor style, structural preferences) and the AI-symptoms they reject, suitable for OTHER modes to consult. Augment mode-palate.md with this summary. Do NOT touch the per-content-set taste/ artifacts — those are owned by palate's own distillation workflow.`,
  },

  layout: "editor",
};

export default palateManifest;
