/**
 * studio-logic — pure, framework-free helpers for the PalatePreview studio.
 *
 ...
 * PalateAddress is assignable wherever an address is expected, while still
 * naming palate's keys for the viewer's own type-safety.
 */
export interface PalateAddress extends Record<string, unknown> {
  contentSet?: string;
  block: string;
  span?: SpanHandle;
  frozen?: boolean;
...
/**
 * Assemble the full palate ViewerAddress for a selection (brief §5.1). A coarse
 ...
 * palate workspace, however, is one content-set directory per writing project
