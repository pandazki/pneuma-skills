/**
 * Palate Mode — complete ModeDefinition.
 *
 * Binds the (React-free) manifest to the ViewerContract: the PalatePreview
...
import PalatePreview from "./viewer/PalatePreview.js";
import palateManifest from "./manifest.js";

/**
 * Palate's address is `{ contentSet?, block, span? }` plus the viewer's
 ...
 */
interface PalateAddress extends ViewerAddress {
  contentSet?: string;
  block?: string;
...
const palateMode: ModeDefinition = {
  manifest: palateManifest,

  viewer: {
    PreviewComponent: PalatePreview,
    ...
    workspace: {
      ...
      // worked-example/) is still surfaced + auto-activated — palate scopes
    ...
    /**
     * Emit the §5.1 viewer-context block for a palate selection. Even a plain
    ...
    extractContext(
      ...
      const address = (selection.address ?? {}) as PalateAddress;
      const contentSet = address.contentSet ?? "";
      const block = address.block ?? "";

      const attrs = [`mode="palate"`];
