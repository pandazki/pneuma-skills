...
describe("skinCssVars — the reading surface is driven by data, not hardcoded CSS", () => {
  ...
  it("emits the palette + geometry as CSS custom properties", () => {
    ...
    expect(vars["--palate-skin-bg"]).toBe(skin.palette.bg);
    expect(vars["--palate-skin-fg"]).toBe(skin.palette.fg);
    expect(vars["--palate-skin-font"]).toBe(skin.fontFamily);
    expect(vars["--palate-skin-measure"]).toBe(skin.measure);
    expect(vars["--palate-skin-line"]).toBe(String(skin.lineHeight));
  });
  ...
  it("a night skin emits a dark background and a day skin a light one", () => {
    ...
    expect(skinCssVars(night)["--palate-skin-mode"]).toBe("night");
    expect(skinCssVars(day)["--palate-skin-mode"]).toBe("day");
  });
});
