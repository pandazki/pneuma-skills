...
// empty root. This pins palate's ACTUAL resolver wiring (allowSingle) against
...
describe("single seeded content-set auto-activation (Bug 2)", () => {
  ...
  it("palate's resolver surfaces the sole subdir as a content set", async () => {
    const palateMode = (
      await import("../pneuma-mode.js")
    ).default;
    const resolve = palateMode.viewer.workspace?.resolveContentSets;
  ...
  it("the lone set auto-activates and the draft then renders its blocks", async () => {
    const palateMode = (await import("../pneuma-mode.js")).default;
    const sets = palateMode.viewer.workspace!.resolveContentSets!(seeded);
