...
import palateManifest from "../manifest.js";

describe("palate manifest — identity", () => {
  it("declares the expected name and version with a matching changelog entry", () => {
    expect(palateManifest.name).toBe("palate");
    expect(palateManifest.version).toBe("0.1.0");
    expect(palateManifest.changelog?.["0.1.0"]).toBeDefined();
    expect(palateManifest.changelog?.["0.1.0"]!.length).toBeGreaterThan(0);
  });

  it("is not hidden and leaves supportedBackends unset (any orchestrator)", () => {
    expect(palateManifest.hidden).toBeUndefined();
    expect(palateManifest.supportedBackends).toBeUndefined();
  });

  it("uses the editor layout and incremental update strategy hints", () => {
    expect(palateManifest.layout).toBe("editor");
  });
});

describe("palate manifest — sources", () => {
  it("declares the five sources from the brief", () => {
    const s = palateManifest.sources!;
  ...
  it("wires draft to aggregate load/save and materials to a read-only glob", () => {
    const draftCfg = palateManifest.sources!.draft.config as {
      load: unknown;
      save: unknown;
    };
    ...
    const matCfg = palateManifest.sources!.materials.config as { patterns: string[] };
    expect(matCfg.patterns).toContain("**/materials/**/*.md");
    expect(matCfg.patterns).toContain("**/materials/**/*.txt");

    const tasteCfg = palateManifest.sources!.taste.config as { patterns: string[] };
  ...
  it("reads crossFamily from the probe-written .pneuma/cross-family.json", () => {
    const cfg = palateManifest.sources!.crossFamily.config as {
      path: string;
  ...
  it("watches .pneuma/cross-family.json so the probe write reloads the banner", () => {
    expect(palateManifest.viewer!.watchPatterns).toContain(".pneuma/cross-family.json");
  });

  it("declares the annotations source and watches draft.annotations.json (the annotation channel)", () => {
    const ann = palateManifest.sources!.annotations;
    ...
    expect(palateManifest.viewer!.watchPatterns).toContain("**/draft.annotations.json");
  });
});

describe("palate manifest — action space", () => {
  const ids = () => palateManifest.viewerApi!.actions!.map((a) => a.id);
  ...
  it("marks every action agent-invocable", () => {
    for (const a of palateManifest.viewerApi!.actions!) {
  ...
  it("assigns the brief's categories", () => {
    const byId = Object.fromEntries(
      palateManifest.viewerApi!.actions!.map((a) => [a.id, a.category]),
    );
  ...
  it("declares the 5 commands from §4.2 with exact ids", () => {
    expect(palateManifest.viewerApi!.commands!.map((c) => c.id)).toEqual([
      "start-from-idea",
      "start-from-draft",
      "request-directions",
      "still-ai",
  ...
  it("supports content sets in the workspace declaration", () => {
    expect(palateManifest.viewerApi!.workspace!.supportsContentSets).toBe(true);
  });
});

describe("palate manifest — init / seeds", () => {
  it("checks content with the draft.md glob", () => {
    expect(palateManifest.init!.contentCheckPattern).toBe("**/draft.md");
  });

  it("declares the three explicit seed cards", () => {
    expect(palateManifest.init!.seeds!.map((s) => s.id)).toEqual([
      "from-idea",
      "from-draft",
      "worked-example",
    ]);
  ...
  it("every seed card's sourceKey resolves to a seedFiles entry", () => {
    const seedFiles = palateManifest.init!.seedFiles!;
    for (const card of palateManifest.init!.seeds!) {
...
describe("palate manifest — agent + evolution", () => {
  it("runs in bypassPermissions and references the cross-family probe in its greeting", () => {
    expect(palateManifest.agent!.permissionMode).toBe("bypassPermissions");
    expect(palateManifest.agent!.greeting).toContain("cross_family_probe.sh");
    // Never instructs the user to "set up their taste" (brief §1, §7.3).
    expect(palateManifest.agent!.greeting!.toLowerCase()).not.toContain("set up taste");
    expect(palateManifest.agent!.greeting!.toLowerCase()).not.toContain("set up your taste");
  });

  it("scopes the evolution directive to the federated summary only", () => {
    const d = palateManifest.evolution!.directive.toLowerCase();
    expect(d).toContain("mode-palate.md");
