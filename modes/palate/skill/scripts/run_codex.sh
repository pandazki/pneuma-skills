...
# palate's cross-family levers: a different RLHF basin than Claude, so it can
...
# --- validate input ---------------------------------------------------------
if [[ $# -lt 1 || -z "${1:-}" ]]; then
  echo "palate: run_codex — usage: run_codex.sh <promptfile>" >&2
  exit 2
fi
promptfile="$1"
if [[ ! -f "${promptfile}" ]]; then
  echo "palate: run_codex — prompt file not found: ${promptfile}" >&2
  exit 2
fi
if [[ ! -s "${promptfile}" ]]; then
  echo "palate: run_codex — prompt file is empty: ${promptfile}" >&2
  exit 2
fi

# --- precondition: CLI present ----------------------------------------------
if ! command -v codex >/dev/null 2>&1; then
  echo "palate: run_codex — codex CLI not installed; degrade to single-family" >&2
  exit 3
fi
...
last_message_file="$(mktemp -t palate-codex-XXXXXX)"
cleanup() { rm -f "${last_message_file}"; }
trap cleanup EXIT

echo "palate: run_codex — generating from ${promptfile} …" >&2
...
if [[ ${status} -ne 0 ]]; then
  echo "palate: run_codex — codex exec failed (exit ${status})" >&2
  exit "${status}"
fi

if [[ ! -s "${last_message_file}" ]]; then
  echo "palate: run_codex — codex returned no final message" >&2
  exit 4
fi
