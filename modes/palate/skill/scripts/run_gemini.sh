...
# role in palate is the neutral third-party judge: with provenance stripped it
...
# --- validate input ---------------------------------------------------------
if [[ $# -lt 1 || -z "${1:-}" ]]; then
  echo "palate: run_gemini — usage: run_gemini.sh <promptfile>" >&2
  exit 2
fi
promptfile="$1"
if [[ ! -f "${promptfile}" ]]; then
  echo "palate: run_gemini — prompt file not found: ${promptfile}" >&2
  exit 2
fi
if [[ ! -s "${promptfile}" ]]; then
  echo "palate: run_gemini — prompt file is empty: ${promptfile}" >&2
  exit 2
fi

# --- precondition: CLI present ----------------------------------------------
if ! command -v gemini >/dev/null 2>&1; then
  echo "palate: run_gemini — gemini CLI not installed; degrade (no third-party judge)" >&2
  exit 3
fi
...
echo "palate: run_gemini — generating from ${promptfile} …" >&2
...
if [[ ${status} -ne 0 ]]; then
  echo "palate: run_gemini — gemini failed (exit ${status})" >&2
  exit "${status}"
fi

if [[ -z "${output//[$' \t\n\r']/}" ]]; then
  echo "palate: run_gemini — gemini returned an empty response" >&2
  exit 4
fi
