...
# write the result to .pneuma/cross-family.json. The wordtaste viewer reads this
...
if ! mkdir -p "${out_dir}" 2>/dev/null; then
  echo "wordtaste: cross_family_probe — cannot create ${out_dir}" >&2
  exit 1
fi
...
work_tmp="$(mktemp -d -t wordtaste-probe-XXXXXX)"
