...
/*
 * wordtaste distillation workflow — the CC-native, Workflow-backed path for the
 * `wordtaste` mode (Taste Writing Studio). The session agent does the cheap
 ...
 * `<skillsDir>/pneuma-wordtaste/workflows/distill.workflow.js`). cosmos's
 ...
 *
 *   Workflow({
 *     scriptPath: ".claude/skills/pneuma-wordtaste/workflows/distill.workflow.js",
...
log(`wordtaste-distill: contentType=${contentType}; reflectors=${reflectorRoster.map((r) => r.family).join(',') || 'NONE'}`)

// pneuma:pure:start
// Deterministic, injected-global-free helpers. Fenced between markers so the
// test suite (modes/wordtaste/__tests__/distill-workflow.test.ts) can extract and
...
const RULES = `
Discipline (wordtaste distillation — from the validated reflect->validate->commit method):
...
if (!candidates.length) {
  ...
  throw new Error('wordtaste-distill: no reflector produced a candidate; cannot distill')
}
