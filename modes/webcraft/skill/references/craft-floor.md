...
- Geometric masks standing in for organic contours. A circle, polygon, or radial-gradient cutout approximating a photographic subject's edge is the cheap version of the effect and reads worse than omitting it — and so is a `clip-path` polygon with many arbitrary vertices, or a curved `clip-path: path()`, drawing a torn edge, blob, or silhouette. Derive an alpha matte from the actual image, or produce a real cut-out asset. `clip-path` stays for geometry: cut corners, diagonals, hexagons.
- A raster buried where it never reaches the screen: a background image under a near-opaque gradient wash, or an image at near-zero opacity. The page ships the wash, and the texture or photo becomes a compliance token. Let the material show (a tint under 0.9 alpha, a blend mode, an opacity you can see) or remove the file.
