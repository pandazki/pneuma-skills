...
Run the `shape` command (see [cmd-shape](cmd-shape.md)), passing along whatever feature description the user provided.
...
If the user has already run the `shape` command and has a confirmed design brief, skip this step and use the existing brief.
...
3. **Check against the DON'T guidelines** in the pneuma-webcraft SKILL.md "Impeccable.style Design Intelligence" section. Fix any anti-pattern violations.
