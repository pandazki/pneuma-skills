...
Ask **one** round of two or three related questions, then move. A precise request needs only a compact confirmation. Skip settled facts, not the confirmation: `DESIGN.md` settles the visual world, not this surface's purpose or concept. {{ask_instruction}}
...
**Before code**, record the chosen direction as a development-only contract under `## Direction contract` in this page's surface brief (`.impeccable/surfaces/<page-slug>.md`, per SKILL.md) — create the brief for it even when no other strategy needs persisting. Five short blocks, roughly 150 words:
...
After writing, read the brief back and confirm all five blocks and the FINISH line are there before you build. If a block reads like a mood, the direction is not decided yet. The brief is what you reread before every edit to the page, and what a later session reloads; a page that looks complete with the FINISH line undischarged is not done, it is abandoned at the finish line.

**Never copy the contract into anything that ships**: no HTML comment, hidden element, `<template>`, `data-*` attribute, meta tag, JSON-LD, script string, or file served beside the page. The page is what the user exports and deploys; an internal strategy note in its source is a leak, and a build step that strips comments is not a safety boundary. Reviewers and the `document` pass receive the contract from the brief.

On a new or replacement world, `DESIGN.md` is written **at finish, from the built page** (step 7) — a rulebook written before the build gets defended against reality instead of describing it. An ordinary extension does not rewrite `DESIGN.md`. Anything else the work settles for this page (scope, mode, the memorable moment, what is still open) goes into the same brief.
...
- **Author the assets; never substitute chrome.** Great surfaces live on carefully made content: names, entries, copy, covers, thumbnails, textures. Gradients, glass, generic icon tiles, and many-vertex `clip-path` polygons where an authored asset belongs are the gap wearing chrome — and a produced texture or photo buried under a near-opaque wash ships the wash, not the material. Icons drawn in the world's own grammar are the remedy, not the target.
...
**Run the review from outside the build.** When the backend exposes a subagent tool, spawn a fresh sub-agent for the finish review with no forked conversation history, and give it: the original request, the confirmed answers, the page path, the capture paths, the direction contract from the surface brief, the approved mock path when one exists, and [craft-floor.md](craft-floor.md). A reviewer that inherits your transcript inherits your framing, your optimism, and your abstractions. Without a subagent tool, run the pass in-thread after stepping fully out of the build context, and say in one line that the review was in-context (`DEGRADED: single-context finish review`) — a substituted review is disclosed, never silent.
...
Then record the world: run the `document` flow ([cmd-document](cmd-document.md)) over what actually shipped, ground truth over intention, and re-run it if any later fix round changes the surface — a `DESIGN.md` describing a layout that no longer exists turns defects into system guidance. Check the outcome by job kind: a new world or an approved system change needs a token-bearing `DESIGN.md` (and its `DESIGN.json` sidecar), not prose alone; an ordinary extension compares the finished page against the incumbent system, leaves those files untouched when they still match, and reports what it checked. Pre-existing drift is reported, not repaired unasked. A clean page is not finished. Finished is the contract kept, the review closed, and the system recorded or confirmed.
