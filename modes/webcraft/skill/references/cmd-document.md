...
**Run the pass from outside the build.** When the backend exposes a Task tool, spawn a fresh sub-agent to record the system, and give it: the project root, the artifact paths, the direction contract (THESIS / OWN-WORLD / STORY / FIRST VIEWPORT / FORM), `PRODUCT.md`, this reference, and the existing `DESIGN.md` when there is one. An agent that inherits the build transcript inherits its intentions and writes them down as facts. Without a Task tool, run the pass in-thread after stepping out of the build context, and say in one line that it was in-context (`DEGRADED: single-context documentation pass`) — a substituted pass is disclosed, never silent.
