---
name: document
description: "Generate a DESIGN.md file at the project root that captures the current visual design system, so AI agents generating new screens stay on-brand. Use when the user asks to extract design tokens, document the design system, or generate a design spec from existing code."
argument-hint: "[scope]"
user-invocable: true
---
...
- New visual work found a coherent incumbent visual system but no `DESIGN.md`.
- The first implementation of a new world is complete and its provisional decisions need to be made permanent — [cmd-craft](cmd-craft.md) §7 hands off here after the last correction lands.
...
## Record from the build, not from the plan

Ground truth is the shipped artifact. Every token and every rule you write must be evidenced by the built code, never by what was planned. Where the direction contract and the build diverge, **the build wins**, and the prose may note the divergence. A rulebook written before the build gets defended against reality instead of describing it.

**Run the pass from outside the build.** When the backend exposes a subagent tool, spawn a fresh sub-agent to record the system, and give it: the project root, the artifact paths, the direction contract (THESIS / OWN-WORLD / STORY / FIRST VIEWPORT / FORM) from the page's surface brief, `PRODUCT.md`, this reference, and the existing `DESIGN.md` when there is one. An agent that inherits the build transcript inherits its intentions and writes them down as facts. Without a subagent tool, run the pass in-thread after stepping out of the build context, and say in one line that it was in-context (`DEGRADED: single-context documentation pass`) — a substituted pass is disclosed, never silent.

Three rules the pass lives or dies on:

- **An existing `DESIGN.md` means update, not replace.** Preserve confirmed incumbent decisions and reconcile them with what shipped. When the pass closes an ordinary extension and the recorded system still matches the build, leave the files untouched and report the evidence you checked; never write merely to prove the pass ran. Drift that predates this build is reported, not repaired unasked.
- **Record durable system rules only.** Tokens the project actually uses, named rules the build actually follows. A token used once is not a system, and a value recorded to make a finding disappear is a defect promoted to doctrine.
- **Never canonize a craft-floor refusal.** An element [craft-floor.md](craft-floor.md) bans — kickers and eyebrow labels, hard offset shadows outside a neobrutalist world, glyph icons, system faces as display voice — is a defect the build carries, not a house style for later surfaces to inherit. One live session shipped five invented kickers and had their styling written into `DESIGN.md`; that is how a single violation becomes the system. Check every prohibition the other way too: a rule that bans a device this world uses natively will be obeyed, and the world loses its own vocabulary to a sentence you wrote in a hurry.

Close the pass with one line naming anything in the build you deliberately did not canonize, and why. Re-run it over the changed surface whenever a later fix round moves that surface: a `DESIGN.md` describing a layout that no longer exists turns defects into system guidance.
...
- **Seed mode**: the project is pre-implementation. Ensure `PRODUCT.md` exists, then reuse the new-work flow's visual-world workshop ([cmd-craft](cmd-craft.md)) and write its directional DESIGN.md seed, marked `<!-- SEED -->`. Re-run in scan mode once there's code.

Decide by scanning first (Scan mode Step 1). If the scan finds no tokens, no component files, and no rendered site, offer seed mode; don't silently switch. An explicit request for seed mode asks for the new-work world workshop, but it does not authorize replacing coherent code: when an incumbent system exists, offer scan mode, or route an explicit identity-replacement request through [cmd-craft](cmd-craft.md).
...
7. **Visible rendered output**: the Pneuma viewer iframe gives a live page right next to the conversation. Sample computed styles from key elements (body, h1, a, button, .card) when needed. This catches values that tokens miss.
...
The following require creative input that cannot be auto-extracted. Ask them in two rounds of no more than three questions each, waiting for the answers between rounds. {{ask_instruction}}
...
- **Shadow Strategy:** [reference the Elevation & Depth section]
- **Border:** [if any]
...
4. The newly-written DESIGN.md is now visible in the Pneuma viewer iframe and will be picked up by subsequent commands when they Read it.
...
### Step 1: Route through the new-work workshop

`PRODUCT.md` is the prerequisite. If it is missing, run the `init` command's product interview first (see [cmd-init](cmd-init.md)). Do not create a visual identity without durable product context.

If `PRODUCT.md` exists, load [cmd-craft](cmd-craft.md) and resolve visual authority there. Seed mode needs a concrete first surface: use the target the user named, or ask what they want to make first. Run cmd-craft §3 "Choose the direction" and §4 "Commit the world", so the visual world and its first expression are chosen together. Stop after the directional DESIGN.md seed and the surface brief; do not implement.

If the new-work flow already completed that workshop in this session, use its chosen direction directly. Do not ask again.

### Step 2: Write seed DESIGN.md

Use the canonical section order from Scan mode. Populate the selected direction and leave unresolved implementation facts as honest placeholders. The seed commits a world and its invariants; it does not pretend implementation tokens already exist.

Lead the file with:

```markdown
<!-- SEED: established with the user before implementation; re-run the `document` command once there's code to capture the actual tokens and components. -->
```
...
- **Colors**: the selected palette strategy and roles, stated as a Named Rule where it earns one (e.g. *"The Drenched Rule. The surface IS the color."*). Include values only when the user, an existing asset, or the direction round established them; otherwise mark them `[to be resolved during implementation]`.
- **Typography**: the selected type character and the relationship between roles. Include font names only when established; otherwise mark the pairing `[to be resolved during implementation]`.
...
2. Tell the user: "Re-run the `document` command (see [cmd-document](cmd-document.md)) once you have some code. That pass will extract real tokens and generate the sidecar."
3. The seed file is now visible in the Pneuma viewer iframe; subsequent commands will read it on demand.
...
- **Be decisive where the evidence is decisive.** Use hard language for actual invariants and softer language for provisional guidance.
- **Use concrete audit tests only when they are grounded in the observed system or a confirmed user decision.** Stitch writes things like *"If it looks like a 2014 app, the shadow is too dark and the blur is too small."* A one-sentence test beats a paragraph of principle.
...
- Don't rename sections even slightly. "Colors" not "Color Palette & Roles". "Typography" not "Typography Rules". Tooling parsing depends on the canonical headers.
