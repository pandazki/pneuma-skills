...
- The user just ran the `teach` command (see [cmd-teach](cmd-teach.md)) and needs the visual side documented.
...
Decide by scanning first (Scan mode Step 1). If the scan finds no tokens, no component files, and no rendered site, offer seed mode; don't silently switch. The user can also explicitly request seed mode regardless of code presence.
...
The following require creative input that cannot be auto-extracted. Group them into one ask interaction. {{ask_instruction}}
...
Before interviewing: "There's no existing visual system to scan. I'll ask five quick questions to seed a starter DESIGN.md. You can re-run the `document` command once there's code, to capture the real tokens and components. OK?"

If the user prefers to skip, stop. No file.

### Step 2: Five questions

Group into one ask interaction. {{ask_instruction}} Options must be concrete.
...
```markdown
<!-- SEED: re-run the `document` command once there's code to capture the actual tokens and components. -->
```
