...
The following require creative input that cannot be auto-extracted. Group them into one ask interaction (use a normal message):
...
4. The newly-written `DESIGN.md` is now visible in the live preview iframe and is read on demand by subsequent commands.
...
Group into one ask interaction (use a normal message). Options must be concrete.
...
3. The seed file is now visible in the live preview iframe; subsequent commands will read it on demand.
