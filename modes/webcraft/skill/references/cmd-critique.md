...
- Assessment A (design review) and Assessment B (pattern catalog + visual evidence) are both required.
- Assessment A and B MUST run as two isolated sub-agents whenever a sub-agent/Task tool is exposed. Running them inline in this context is "possible" but is NOT permitted; it is a degraded run. Inline is allowed ONLY when no sub-agent tool exists (or the user declined, on backends that ask).
...
1. **Resolve the target** to a concrete file path or URL:
   - "the homepage" → `<content-set>/index.html`
   - "the settings modal" → the primary component file
   - "this page" → the active page from `<viewer-context>`
...
- Spawn A and B as two isolated, parallel sub-agents whenever a sub-agent/Task tool is exposed (e.g. Claude Code's Task tool). This is the default and is mandatory; do not run them inline because it is faster.
- "Unavailable" means exactly one thing: no sub-agent tool is exposed in this session (or, on backends that ask, the user declined). It does not mean inconvenient.
...
On Codex backends (whose permission model requires asking before spawning): asking is the normal path, not a degradation. If sub-agent spawning is exposed but the user has not explicitly allowed it, ask exactly once: "Impeccable critique is designed to run two independent sub-agents for an unanchored assessment. May I use sub-agents for this critique?" Then stop until the user answers. If allowed, spawn A and B. If declined, run sequentially and lead the report with `⚠️ DEGRADED: single-context (sub-agents declined by user)`. If spawning is not exposed, do not ask; run sequentially with the degraded banner.
...
Where the target is a viewable page, supplement the file scan with visual evidence: use the `capture` viewer action to screenshot the rendered page (full and per-section as needed) and note visually detectable tells (contrast failures, text overflow, layout breakage at the current viewport).

Return: file-based findings catalog with counts and locations, visual-evidence notes, and any ambiguous cases flagged as potential false positives.

After Assessment B returns usable findings, reuse them. Do not redo the scan in the parent unless Assessment B failed, was truncated, or omitted counts, rule names, or file locations.
...
Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `{{config_file}}` contains a `## Design Context` section from a prior `init` run, also generate 1-2 project-specific personas from the audience/brand info.
...
If `{{config_file}}` contains a `## Design Context` section (generated by the `init` command), derive 1–2 additional personas from the audience and brand information:
