...
**Output**: A design brief that can be handed off to the `craft` command (see [cmd-craft](cmd-craft.md)), the pneuma-webcraft SKILL.md "Impeccable.style Design Intelligence" section, or any other implementation flow. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
Discovery includes at least one user-answer round unless PRODUCT.md, DESIGN.md, `.impeccable.md`, or an already-confirmed brief directly answer the needed inputs. With a sparse prompt, do **not** synthesize a complete brief for confirmation on the first response.

- Ask **2-3 questions per round**, then wait for answers.
- Treat PRODUCT.md, DESIGN.md, and `.impeccable.md` as anchors; they reduce repeated questions but do **not** replace shape for craft. Shape is task-specific.
...
Force a visual decision on three fronts. Skip anything PRODUCT.md, DESIGN.md, or `.impeccable.md` already answers; ask only what's missing.
...
- Image generation is enabled in this Pneuma session (the `generate_image.mjs` script is available — the pneuma-webcraft SKILL.md describes its use).

When those conditions are met, this step is mandatory. If image generation isn't available, do not ask the user to install APIs or tooling. State in one line that the image step is skipped because image generation isn't enabled in this session, then proceed. The one-line announcement is required, not optional; it forces a conscious decision instead of letting the step quietly evaporate.
...
If image generation isn't enabled, announce the skip in one line and proceed to the design brief.

## Phase 2: Design Brief

After the interview and any required probes, present a brief and **end your response**. **STOP — the user must confirm before any implementation runs.** Do not present a brief and then continue to code in the same response, even if the brief feels obvious to you. The user's confirmation is the gate.
...
**3. Design Direction**
Color strategy (Restrained / Committed / Full palette / Drenched) + the theme scene sentence + 2–3 named anchor references. Reference PRODUCT.md and DESIGN.md (or `.impeccable.md`) where they already answer, and note any per-surface overrides.
...
Based on the brief, list which reference files would be most valuable during implementation (e.g., cmd-layout.md for complex layouts, cmd-animate.md for animated features, interaction-design.md for form-heavy features, cmd-typeset.md for typography-driven pages, cmd-colorize.md for color-led brands).
...
Once confirmed, the brief is complete. The user can now hand it to the `craft` command (see [cmd-craft](cmd-craft.md)), or use it to guide any other implementation approach.
