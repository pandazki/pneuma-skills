...
**Output**: A design brief that can be handed off to the `craft` command (see [cmd-craft](cmd-craft.md)), the pneuma-webcraft SKILL.md "Impeccable.style Design Intelligence" section, or any other implementation skill.
...
Once confirmed, the brief is complete. The user can now hand it to the pneuma-webcraft SKILL.md "Impeccable.style Design Intelligence" section, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use the `craft` command (see [cmd-craft](cmd-craft.md)) instead, which runs this skill internally.)
