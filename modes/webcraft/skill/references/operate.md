...
When design SERVES the product: app UIs, admin dashboards, settings panels, data tables, tools, authenticated surfaces, anything where the user is in a task. The essentials live in SKILL.md's Modes section and [craft-floor.md](craft-floor.md); this file is the extended depth, written for Operate surfaces. Read surfaces (docs, guides, long-form) take SKILL.md's Read mode plus this file's typography and consistency rules; their prose measure and navigation matter more than component density. Forms, focus order, and loading patterns have their own depth in [interaction-design.md](interaction-design.md).
