...
This is the highest-leverage option. **One composite image generated by gpt-image-2.5-sunburst contains all N panels in a grid**, then `scripts/storyboard.mjs` slices the composite into N individual PNG files. Each slice becomes a previewFrame asset on the timeline, AND can be used directly as a reference image for seedance (Path C in `references/storyboard-workflow.md`).
...
2. **Internal consistency is dramatically higher** — gpt-image-2.5-sunburst spends one image's worth of attention budget across all N panels at once, so character look, palette, and lighting hold across the grid in a way independent generations cannot.
...
| 13-16 panels | C | Last grid size gpt-image-2.5-sunburst handles cleanly |
...
Cost is one image generation plus local slicing; read `usage.cost` from the OpenRouter response instead of assuming a fixed per-image price.
...
composite via Sunburst (or Flare with references), then computes panel bounding boxes from
...
- **Forgetting to cite the palette / character / shot type in the sketch prompt** — the design layers exist in your notes but not in the gpt-image-2.5-sunburst prompt. Re-read every sketch prompt before invocation: does it carry the panel template's full context?
