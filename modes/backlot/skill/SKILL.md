...
description: >
  ...
  the shot plan, blocking a shot as a Blender greybox, checking it, rendering
  the storyboard's key frames from that greybox, writing a prompt pack,
  generating and checking a video take, recording dialogue and music, and
  cutting the film. Defines the eight stages and the order they run in, the
  approval gate in front of every paid step, the two scripts that own the
  machine state, and what may be called finished. Consult before your first
  action in a new conversation.
---
...
with every beat's picture written down; each shot is blocked in 3D; each block
is rendered into the storyboard's key frames and then into a take a video model
paints; the takes become a cut. Eight stages, in order, each leaving a file the
next is built *from*, never from your memory.

The **previz department is the heart of this studio**, and it works in one
order: **design the picture in words, then block it, then render the picture
from the block, then buy it.** What each beat looks like is written into the
shot plan first; then an untextured Blender animation — a *greybox* — fixes the
space, who is where and when, what the props do and what the camera does; then
its frames are re-rendered in the film's real look as this shot's **key
frames**, which *are* the storyboard; and only then does a video model see any
of it. **Nothing is drawn beside the greybox.** Round 3 drew a storyboard frame
per shot from the text before anything was blocked: the eight pictures shared
no space and no camera, and the greybox could not satisfy any of them. The
greybox is the one place this film's space and camera exist, so every picture
is derived from it. Greybox people are
...
| `boards` | the **shot plan** strip: scene number, one line, seconds, the shot's stage badge, and whatever picture that shot has yet — its key frame, else its greybox, else a grey card. Nothing is drawn here |
| `previz` | the shot's player (below), default lane `greybox`; the panel's **Lineup** tab stands each key frame beside the greybox second it was rendered from, with the beats and the continuity note |
...
clock: `board` (a legacy still, only on a film that drew one), `reference` (the
segment being recreated, recreate only), `greybox` (your Blender animation —
*Render* is the MP4 the model
...
1. **Eight stages, in order, each leaving a file.** `idea` → `idea.md`;
   ...
   + `shot-plan.md`, and **no image**; `previz` → `shots/<id>/greybox/**` +
   `anchors/**`; `takes` → `shots/<id>/takes/**`; `sound` → `sound/sound.json`
   + the line MP3s; `cut` → `cut/{final.mp4,edl.json}`. Build each stage from
   the previous stage's file.
...
10. **The design is the plan's text; the pictures are derived from it through
    the greybox.** What a beat looks like is written at the shot-plan stage,
    in words, before anything is built — and it is not drawn there. The
    greybox is built from those words, the key frames are rendered from the
    greybox, and the prompt is that same design carried forward: never
    composed at the takes stage, never shortened. The next section is how it
    is written.
...
13. **Cost lives beside the artifact, and you quote it in dollars.** Every paid
    call — sheet, concept, key frame, take, TTS line, music — is recorded
...
| **【素材映射】** | one line per attached reference, at the index `generate` will actually attach it, each with a positive scope **and** an explicit exclusion (`只参考…，不用…`), plus which greybox block is which character. `@Image1` is the **`first` key frame** — 开场构图、机位、人物位置与画风，本镜的分镜稿，由 @Video1 的同一帧渲染而来；动作照时间戳与白模. There is **no board line**: a board is attached only on an old film that has one and no key frame. The `@Video1` line must disinherit the grey material, the empty set, the block shapes and the viewport overlays, or the model paints grey |
...
| 3 | `bible` | `screenplay.md`, the scenes | the look and voice descriptions | `backlot.mjs character add\|look\|voice`, `set add\|look` | blocking the shots |
| 4 | `boards` — **the shot plan, and it is free** | the bible, the scenes | `shot-plan.md`, the beats **with their `detail`**, the entry/exit and continuity decision. **No image is made here** | `previz.mjs meta\|beats`, `previz.mjs lines --set` | building greyboxes (free) |
| 5 | `previz` | the shot plans | `greybox/scene.py` | `previz.mjs render`, `check`, then `anchor` (the key frames) and `lineup` | paid takes |
| 6 | `takes` | the final greyboxes and their key frames | `prompts.md`, from `prompt-skeleton` | `previz.mjs generate`, `check`, `select` | paid VO and music |
...
design, never photoreal: these images travel into every key frame and every
take, and a photoreal face is what the likeness filter refuses.

**4. The shot plan — design the picture in words, and draw nothing.** Read
`references/shot-plan.md`. This stage is free and produces no image. Per shot:
write `shot-plan.md` with the timeline, the entry and exit states and the
continuity decision for the cut into it; register the beats with a **`detail`
on each** — the designed picture of those seconds, which is what the greybox
is built from and what the take's prompt is made of — plus the lines, the ties
and any hand-off (`previz.mjs beats`, `lines --set`, `meta --scene
--characters --set --continues-from --entry --exit`). Then show the creator
the shot list and stop. **Do not generate a frame here.** Round 3 proved that
storyboard frames drawn from the text before the greybox exists contradict
each other — no shared space, no shared camera — and the greybox can satisfy
none of them; the pictures come from the greybox in the next stage.

**5. Previz — block it, then render the storyboard from the block.** Read
`references/greybox.md` — the kit, the build order and the acceptance list —
and `references/camera.md` before any camera block. Per shot: layout →
blocking → prop events → camera (one move) → tempo, each layer checked before
the next hides its mistakes. `render --preview`, look at sheets and strips,
`check` every item, fix what fails, then `render` for the final `greybox.mp4`
+ `scene.blend` + `scene.glb`. Blender is only ever started by `previz.mjs
render`, headless; never ask the creator to open it.

**Then the key frames — they are the storyboard.** A passing greybox is
correct and unreadable, so re-render its frames in the film's real look:
`previz.mjs anchor <shot-dir> [--id first|key] [--at s]` is an image-to-image
call whose composition, framing, positions and scale are the greybox frame
exactly, whose appearance is the sheets and the concept only, whose style is
the film's style reference when one is registered (`backlot.mjs style
--keyframe`), and whose prompt is that second's beat `detail`. `first` is the
frame the take opens on; add a second one (`--id key`) for a moment the shot
turns on. Look at `previz.mjs lineup <shot-dir>` — each key frame beside the
greybox second it came from — and fix whatever disagrees, usually the key
frame's prompt. Then present the stage as one review: per shot the lineup, the
beat details, the continuity decision and the seconds; across the film, the
reel from `backlot.mjs cut --reel`, which stands the greybox in for every
missing take. Free, ungated, and the first time anybody sees whether the film
*works* — say plainly that it is a reel. Bible, key frames, greybox and
breakdown are approved **together**, and that is the last free moment: a
pacing problem found here costs nothing, found later costs takes.
...
| `style <project> --keyframe <png>` \| `--clear` | the film's **style reference**: one picture that says how this film is rendered — idiom, palette, light, finish — and nothing about what is in the frame. Copied into the project; `previz.mjs anchor` then attaches it to every key frame as the last reference. Free |
...
| `anchor <shot-dir> [--at 0] [--id first] [--prompt "…"]` · `lineup <shot-dir>` | the **key frame**, and it comes from the greybox: the final greybox's frame at `--at`, re-rendered in the film's look — composition, framing, positions and scale from that frame exactly, appearance from the sheets and the concept only, style from the film's style reference. `lineup.png` stands each key frame beside the greybox second it came from, with the beats underneath. `anchor` is paid and gated on `bible` plus a final greybox; `lineup` is free |
| `board <shot-dir> --file board.png --prompt "…"` | **legacy, not part of the flow** — registers a drawn board on a film shot under the old order. A shot that has a key frame never sends its board to a take |
...
| `generate <shot-dir> [--resolution 480p] [--fix "…"] [--user-approved] [--allow-failing "…"] [--no-handoff] [--estimate] [--audio]` | price, then run Seedance with the greybox, the key frames, the bible and the voice references attached — plus the previous shot's out-frame when this one hands off, which is why it refuses until that shot has a selected take (`--no-handoff` overrides and is recorded). `--estimate` lists what it would attach and stops. When it attaches images or audio it prints a `priceNote`: the table does not price those, so the recorded figure is the table's, **not a bill** — quote it that way |
...
character sheets and set concepts; `--image-urls` takes local paths and is the
continuity mechanism (it is also how `previz.mjs anchor` renders a key frame
image-to-image from a greybox frame). The other four are invoked *for* you: `generate-tts.mjs`
...
| `generate-take` | confirm a final render and a key frame exist and no greybox check is failing, show the `--estimate` and the pack's `prompt` block, then `generate`. Their click is authority for one take at the draft resolution, not for a series |
...
| The Blender kit, pawns, build order, the key frames and the lineup, the acceptance list | `references/greybox.md` |
