...
cast with designed faces, a look for the film and places described in words;
those become a shot list with every beat's picture written down; each shot is
blocked in 3D; each block becomes a take a video model paints; the takes become
a cut. Eight stages, in order, each leaving a file the next is built *from*,
...
timing and the camera. So: **space, blocking, prop events and camera live in
the greybox; body action, look and tempo live in the prompt**, carried forward
from the plan rather than composed at the end.
...
clock: `board` (a legacy still, only on a film that drew one), `reference`
(recreate only), `greybox` (your Blender animation — *Render* is the MP4 the
model receives, *3D* the same scene in the browser) and `take` (a generated
video; a shot can hold several), over a beats timeline with a `tempo` row
wherever the greybox warps time. Every message the creator sends carries a
`<viewer-context mode="backlot">` block saying where they are — `stage`,
`shot`, `lane`, `take`, `time`, `frame`, the `beat` the playhead is inside and
`range` when they marked one. "Here" means that moment: go and look at it.
...
- `navigate-to { address }` — put the stage where you are talking about: when
  ...
  playhead there; add `range` for the span you checked), and after a take
  lands (`{ stage: "takes", shot, take: "take-02", layout: "side" }`). An
  unknown shot or take is refused by name.
...
`compare`, `lineup`); a frame you have not opened you cannot describe. The
viewer never writes to the workspace, and the approve button is a *message to
...
1. **Eight stages, in order, each leaving a file.** `idea` → `idea.md`;
   ...
   Build each from the previous stage's file.
2. **Stop at the end of every stage and show what you made.** `navigate-to`
   ...
   then report every stage: their judgement on stage 2 beats four stages built
   on the wrong story.
...
9. **No limbs in the greybox, and props at their real size.** Subjects are
   ...
   greybox shows only the spatial consequence (the door swings, the button
   dips). A door is 2.0–2.1 m, a counter 0.9 m, a fridge about 2 m, a stone
   step 0.15 m: the model reads a person's height off the props, and blind
   trial 2 stood an adult next to an over-tall cooler and got a child back.
10. **The design is the plan's text; the one picture is the greybox.** What a
    beat looks like is written at the shot-plan stage, in words, before
    anything is built — and it is not drawn there. The prompt is that same
    design carried forward: never composed at the takes stage, never
...
text-to-video guides is what made the second acceptance run delete its own
designed beats. Carry the whole design; cut vagueness, never length, and put
the non-negotiables first and the prohibitions last. `prompt-skeleton` emits
this block order, in the film's language:
...
`previz` → paid takes; `takes` → paid voice-over and music; `sound` → the
final cut.
...
arithmetic in `references/shot-plan.md`, the cut into each decided here — and
register them (`shot add`). That moves the `boards` stage, not this one.
...
never photoreal: a photoreal face is what the likeness filter refuses. **A
place is described, not drawn** — `set add --look "…"` carries materials,
colours and **real metres** into every pack; a concept frame is optional.
...
`shot-plan.md` with the timeline, the entry and exit states and the continuity
decision for the cut into it; register the beats with a **`detail` on each** —
the designed picture of those seconds, which the greybox is built from and the
prompt is made of — plus the lines, the ties and any hand-off (`previz.mjs
beats`, `lines --set`, `meta --scene --characters --set --continues-from
--entry --exit`). Show the shot list and stop. **Do not draw here**:
storyboards made before the greybox contradict each other, and the block can
satisfy none of them.

**5. Previz — block it, check it, show the reel.** Read
...
Then present the stage as one review: per shot the greybox, the beat details,
the continuity decision and the seconds; across the film, the reel from
`backlot.mjs cut --reel`, which stands the greybox in for every missing take —
...
template from the record — the reference lines with their exclusions, the
...
Read the report: `trimmed` holds exactly the shots you trimmed, `droppedVo` a
line whose second fell outside its trim — move it or widen the trim, never
leave one unmentioned. Then deliver `final.mp4`, `edl.json`, the takes, the
packs, the acceptance records and the cost total, each named for what it is.
...
`generate-bgm.mjs`, `transcribe.mjs` and `seedance-video.mjs` — that last one
only ever through `generate`.
...
- `generate-take` — confirm a final render exists and no greybox check is
  failing, show the `--estimate` and the pack's `prompt` block, then
