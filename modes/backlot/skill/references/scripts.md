...
`previz.mjs` is the **shot**: `shots/<id>/shot.json`, its beats, lines, board
record, greybox revisions, acceptance record and takes. Neither writes the
...
| `cost <project>` | every paid call the files record — sheets, voices, boards, takes, voice-over, music — totalled by stage and by kind. A call with no recorded price is listed as unpriced, never as free |
...
generating a sheet or a concept, and `gate <project> board` before a board
frame.
