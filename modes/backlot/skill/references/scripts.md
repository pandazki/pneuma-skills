...
| `status <project>` | the whole film: every stage with its status, cost and approval time; the gates; the scenes, the bible, the shots with their takes and lines; the sound and the last cut; `unregistered`, the videos in `cut/` the Cut stage does not show; the first open stage. Never writes |
...
| `set add\|set\|look <project> <id> …` | the places (a "set" is a place) — same grammar as `character`, `set set` amends. **`--look` is the set's picture**: it is pre-filled into the pack's 【全局设定】 and is the only thing that tells a take what the place is made of, so `set add` warns without one. `set look` (the concept frame) is optional, is not attached to a take unless `generate --with-concept` asks for it, and never overwrites a written `look` |
| `style <project> --keyframe <png> [--prompt "…"]` \| `--clear` | the film's **style reference**, and it is ESSENTIAL: one picture that says how this film is rendered — idiom, palette, light, finish — and nothing about what is in the frame. Copied to `style/keyframe.png` and recorded in `backlot.json.style`; every take carries it as an `@Image` (so does every key frame `previz.mjs anchor` renders), with the job "only the idiom, never the composition". `generate` warns when a film has none. With no flags it prints the one that is registered. Free, ungated |
...
| `cut <project> --reel\|--final [--music-db=-18] [--music-fade 2]` | assemble the film, honouring every shot's trim for both take and greybox sources, and write `cut/edl.json` last. `--reel` stands the greybox in for any shot without a selected take and labels those segments; a **free** shot with no greybox gets a black card with its title for its own seconds (`source: "card"`, counted in `standIns` and listed in `cards`) — it never owed the reel a block; `--final` refuses while any shot lacks one, and is gated on `sound`. A negative decibel needs the `=` spelling: `--music-db=-22`. Run after a registered finish, either one reports `supersededFinish` |
| `cut <project> --finish <film> --by "…" [--edit <edit.json>]` | register a **finishing pass** (titles, captions, UI, transitions, a re-mix, all made outside the cut) as the film. It needs the current `edl.json` to be a `--final` assembly with no stand-ins, whose takes are still the selected ones, and the `sound` gate. It probes the film, copies it to `cut/finished.mp4` and rewrites `edl.json` last: `file: "finished.mp4"` plus `finish: { by, source, retimed, assembly }`, where `assembly` is the plain cut's own edit list. Without `--edit` the film must be as long as the assembly, within a frame, and inherits its segments. A pass that changed the timing passes `--edit` with `{ segments, vo?, music? }` in `edl.json`'s shape on the finished film's clock. Each segment's `source` must be the shot's selected take. Omitted shots are listed in `omitted` |
...
| `boards` | the shot order + each shot's `title, scene, characters, set, conditioning, spec, beats` (their `detail` included), `board, trim, continuity`. Conditioning is a plan decision, so shooting a shot free re-opens the shot list. The stage makes **no image** — `board` is null on every film shot under the current order, and the stage is still defined, hashed and approvable without one |
| `previz` | each shot's final greybox revision + its greybox check statuses + its key frames (`anchors`), when a shot has any — most have none, and the stage is approvable without them |
...
| `anchor` (a shot's OPTIONAL key frame) | `bible` — plus a final greybox to render it from |
...
| `meta <shot-dir> --conditioning greybox\|free\|hybrid` | **how this shot is made** (`shot-plan.md`). `greybox` (the default, and what a shot written before the field means) sends the block as `@Video1`; `free` sends no video at all — the sheets and the style frame are the whole reference set, no greybox and no greybox checks are required, `next` goes plan → prompt, the job is priced on the no-reference row, and `take-motion`/`take-camera` are asked about the PLAN; `hybrid` sends the block and adds the sentence that lets the body and the camera move inside it. Switching to `free` drops the greybox checks **nobody answered** (they have no subject any more) and keeps every recorded verdict; switching back re-seeds the list. Free is not a downgrade — it is where the fight and charm beats live |
| `meta <shot-dir> [--scene --characters --set] [--trim-in 0.4 --trim-out 1.6] [--no-trim] [--continues-from <shot>] [--entry "…"] [--exit "…"] [--no-continuity]` | where the shot sits in the film — the list `generate` and `anchor` read to attach the right sheets and voices. `""` clears an id; an id the bible does not carry yet is a warning, not a refusal. `--trim-in`/`--trim-out` name the sub-range of this shot's own clock the **cut** uses (`0 ≤ in < out ≤ the spec's seconds`); one flag alone edits the range that is there, and `--no-trim` clears it so the whole shot reaches the film again. `--continues-from` declares the hand-off (an **earlier** shot in `backlot.json.shots`), `--entry`/`--exit` are the first and last half second in words — an `--exit` alone is legitimate and is how the shot before a hand-off says what the next one opens on, and the pair is **the whole hand-off the model sees**, since the frame itself is only attached with `generate --with-handoff` — and `--no-continuity` clears the block |
...
| `board <shot-dir> --file <png> --prompt "…" [--refs a.png,b.png] [--cost-usd --cost-basis]` | **LEGACY — not part of the flow.** Registers a drawn board on a film shot under the old order; copies it to `board.png`, bumps its revision, records the prompt, refs and cost. Gated on `bible`. Boards drawn before the greybox contradicted each other and the greybox could not satisfy them (`shot-plan.md`), so nothing is drawn now — and a board reaches a take only through `generate --with-board` |
...
| `anchor <shot-dir> [--at 0] [--id first] [--prompt "…" \| --prompt-file <f>] [--aspect-ratio 16:9] [--quality high] [--cost-usd --cost-basis]` | **OPTIONAL — a picture for the creator, not a reference the take receives unless `generate --with-anchors`.** Cuts the *final* greybox's frame at `--at` and hands it to `generate_image.mjs` as the composition, camera, positions and scale, together with this shot's character sheets and its set concept when it has one for appearance ONLY and the film's style reference (`backlot.mjs style`) LAST for the idiom, prompted from the beat `detail` at that second. Writes `anchors/<id>.png` and records `{ id, at, file, revision, prompt, refs, cost }`; re-running an id bumps its revision. Use it to SEE a shot before buying the video. Paid, priced from the vendor's own `usage.cost`; gated on `bible` plus a final greybox. Part of the `previz` stage's content |
| `lineup <shot-dir> [--at s] [--id first] [--out <path.png>]` | **OPTIONAL and free**: every key frame beside the greybox frame it was rendered from, with the beats written underneath — again, a picture for the creator. One greybox frame per key frame at that key frame's own second, unless `--at` names one moment for all of them; `--id` narrows it to one key frame; a legacy board is shown last and labelled. Whatever is missing is left out and named |
...
| `prompt-skeleton <shot-dir> [--write] [--with-anchors] [--with-board] [--with-concept] [--with-handoff]` | the whole prompt pack, pre-filled from the record and in the block order `prompting.md` documents: the replacement sentence, 【素材映射】 with one scope-and-exclusion line per reference at the index `generate` will attach it, 【一句话成片】 with this shot's seconds and aspect, 【全局设定】 carrying the camera beat's designed sentence whole, 【时间戳分镜】 as a **contiguous** partition of the clip (no gaps, no overlaps, ≈1 segment per 1–1.5 s, each carrying its beat's `detail` in full, denser beats merged rather than shortened), the entry/exit frames — the 第一帧 line opening `承接上一镜（sXX）的结束状态：` and the global block gaining `机位与景别以本镜白模 @Video1 为准，不沿用上一镜的机位。` on a continuing shot whose frame is not attached — the spoken lines at their seconds, 声音 with `不要配乐`, the regeneration line and 【全局锁】 last. Chinese scaffolding for a film whose `screenplay.md`/`idea.md` is CJK, English otherwise; tags stay `@Video1/@Image1/@Audio1`. **No word budget.** The text comes back in the JSON's `skeleton` with `language`, `segments`, `maxSegments` and `merged`; `--write` puts it in **`prompts.skeleton.md`** and never touches `prompts.md` — copy the filled block in yourself. The `--with-…` flags mean what they mean on `generate` and must MATCH the call that will run, because the pack has to assign exactly the references the job attaches; the JSON echoes them as `attachments`. Free, and the way every pack starts |
...
| `generate <shot-dir> [--resolution 480p\|720p] [--seconds n] [--fix "…"] [--user-approved] [--allow-failing "…"] [--no-handoff] [--estimate] [--audio] [--timeout 1800] [--with-anchors] [--with-board] [--with-concept] [--with-handoff]` | price, then run Seedance 2.5 reference-to-video with the references gathered from the shot's record — the greybox, the character sheets, the film's style key frame and the voices, plus whatever a `--with-…` flag asks for (see `video-generation.md`) and the first fenced `prompt` block of `prompts.md`. Refuses while the film's `previz` stage is not approved, without a final greybox at the current revision, while a greybox check is failing (unless `--allow-failing "<reason>"`), a second take without `--fix`, a third or later without `--user-approved` as well, a prompt naming a reference index nothing was attached at, **a prompt that leaves an attached reference unassigned**, or — when the shot declares `continuity.from` — while the shot it continues has no selected take (`--no-handoff` generates out of order, cuts nothing and records `"skipped"` on the take; `--with-handoff` together with `--no-handoff` is refused). With a hand-off it cuts the previous take's frame at that shot's trim `out` into `takes/handoff-in.png` and records it on the take as `handoff: { …, attached }` — the frame is the evidence `compare --handoff` and `take-handoff` use, and it is **sent to the model only with `--with-handoff`** (last image reference, `role: "handoff"`); otherwise the join travels as the pack's 第一帧/最后一帧 text, because an attached out-frame brought the previous shot's camera with it in every continuing shot of the eight-take run (2026-09-21 night). An assignment may use `=`, `:`, `：` or `is`. **Warned about, never refused:** a timeline line past the shot, running backwards, leaving a gap or overlapping the one before it; a segment naming two camera moves; a beat whose designed `detail` no timeline line carries any more; a missing 【全局锁】/【Locks】 block; an `@Video1` line with no exclusion; an unfilled `<TODO: …>`; **a film with no style key frame**; a legacy board attached beside a key frame. `--estimate` prices the job, lists what would be attached, and stops. Otherwise the take is recorded `submitted` — with the prompt at `takes/<id>.prompt.txt` — before the request leaves, and ends `done` or `failed`. A shot with a spoken line is generated with audio and transcribed on arrival to `takes/<id>.transcript.json`; `take-lines` passes only if every line is in it. When images or audio are attached it prints a `priceNote`: the table prices the output and the video reference only, so the recorded figure is the table's and **not a bill** |
...
**There is no picture step at all** — no drawing before the blocking and no
key frame after it. An accepted greybox goes straight to the pack, and
`anchor`/`lineup` are optional pictures for the creator that do not move
`next` either way. It is a report, not a gate: read it when you resume a session or lose the
