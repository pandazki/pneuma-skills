...
const artwork = resolve(process.argv[2] ?? ".tmp-illustrate-showcase");
const fonts = resolve(here, "../../../public/fonts");
const server = Bun.serve({
  hostname: "127.0.0.1", port: Number(process.env.SHOWCASE_PORT ?? 18142),
  async fetch(req) {
...
console.log(`Showcase: ${server.url}?view=hero (also text-to-image, row-canvas, region-edit)`);
